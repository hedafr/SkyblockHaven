...
import de.skyh.skyhaven.command.exception.ThrowingConsumer;
import de.skyh.skyhaven.config.CredentialStorage;
import de.skyh.skyhaven.data.Friend;
import de.skyh.skyhaven.data.HyPlayerData;
import de.skyh.skyhaven.data.HySkyBlockStats;
import de.skyh.skyhaven.data.HyStalkingData;
import de.skyh.skyhaven.error.ApiHttpErrorEvent;
import de.skyh.skyhaven.error.ApiHttpErrorException;
...
public class ApiUtils {
    ...
    private static BufferedReader makeApiCall(String url, boolean sendApiKey) throws IOException {
        return makeApiCall(url, sendApiKey, CredentialStorage.skyh);
    }

    private static BufferedReader makeApiCall(String url, boolean sendApiKey, String key) throws IOException {
        if (sendApiKey && !CredentialStorage.isSkyhValid) {
            throw new ApiHttpErrorException("Your current Hypixel API key is invalid. Use §4/skyh apikey §cto manually set your API key.", url, true);
        }
        try {
            ...
            if (connection.getResponseCode() == HttpStatus.SC_NO_CONTENT) { // http status 204
                ...
                throw new ApiHttpErrorException("Your current Hypixel API key seems to be invalid. Use §4/skyh apikey §cto manually set your API key.", url, true);
            ...
            if (cause instanceof ValidatorException && cause.getCause() instanceof SunCertPathBuilderException) {
                throw new ApiHttpErrorException("" + EnumChatFormatting.DARK_RED + EnumChatFormatting.BOLD + " ! "
                        + EnumChatFormatting.RED + "Java is outdated and doesn't support Let's Encrypt certificates (out of the box). A game restart might fix this issue. If the problem persists, open a ticket on the SkyhX discord server.", Skyhaven.INVITE_URL);
            } else {
