...
import de.skyh.skyhaven.util.TickDelay;
 ...
 * Based on GuiModList
 */
public class SkyhConfigBestFriendsMigration extends GuiScreen {
    /**
     * <pre>
     * §9§m-----------------------------------------------------
    ...
    public void onGuiClosed() {
        ...
        if (Skyhaven.getInstance().getFriendsHandler().getBestFriendsListSize() == 0) {
            new TickDelay(() -> Skyhaven.getInstance().getChatHelper().sendMessage(new SkyhChatComponent("[§2Skyhaven§a] All Skyhaven best friends have been migrated or removed.").green()
                    .appendFreshSibling(new SkyhChatComponent("You can now use Hypixel's commands instead: §b[open patch notes]").green().setUrl("https://hypixel.net/threads/social-update-online-status-best-friends-more.4638020/")
                            .appendFreshSibling(new SkyhChatComponent(" §6➊ §aView best friends list: §2/friend list best §aor §2/fl best").green().setSuggestCommand("/fl best"))
                            .appendFreshSibling(new SkyhChatComponent(" §6➋ §aAdd or remove best friend: §2/friend best <player name>").green().setSuggestCommand("/f best ")))),
                    isMigrating ? 20 : 0);
        }
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        this.drawString(this.fontRendererObj, "Skyhaven: Best friends migration", (int) ((this.width / 2 + 8) / scaleFactor), 6, 0xFFCC00);
        ...
        if (btnRemoveAll.isMouseOver()) {
            ...
            removeAllTooltip.add(EnumChatFormatting.RED + "Discard/Remove " + EnumChatFormatting.DARK_RED + "all " + EnumChatFormatting.RED + "Skyhaven best friends");
            drawHoveringText(removeAllTooltip, mouseX, mouseY);
        } else if (btnHelp.isMouseOver()) {
            List<String> helpTooltip = new ArrayList<>();
            helpTooltip.add("" + EnumChatFormatting.GOLD + EnumChatFormatting.BOLD + "Skyhaven: Best friends migration");
            helpTooltip.add(EnumChatFormatting.GREEN + "If you have any questions or need help with the best friends list migration:");
            helpTooltip.add(EnumChatFormatting.GREEN + "Join the SkyhX discord and open a ticket!");
    ...
    protected void actionPerformed(GuiButton button) {
        if (button == btnRemoveAll) {
            ...
            GuiYesNo guiDeleteRule = new GuiYesNo(SkyhConfigBestFriendsMigration.this,
                    EnumChatFormatting.RED + "Discard/remove " + EnumChatFormatting.DARK_RED + Skyhaven.getInstance().getFriendsHandler().getBestFriendsListSize() + " (= all)" + EnumChatFormatting.RED + " from Skyhaven best friends list?",
                    EnumChatFormatting.RED + "This action cannot be reverted!",
                    1337);
            mc.displayGuiScreen(guiDeleteRule);
        } else if (button == btnHelp) {
            GuiConfirmOpenLink guiHelp = new GuiConfirmOpenLink(this, Skyhaven.INVITE_URL, 9001, true);
    ...
    public void confirmClicked(boolean result, int id) {
        ...
        if (result) {
            if (id == 1337) {
                Skyhaven.getInstance().getFriendsHandler().removeAllBestFriends();
            } else if (id == 9001) {
                try {
                    Desktop.getDesktop().browse(new URI(Skyhaven.INVITE_URL));
                } catch (Throwable throwable) {
                    Skyhaven.getInstance().getLogger().error("Couldn't open link " + Skyhaven.INVITE_URL, throwable);
                }
            } else if (id < 500) {
    ...
    private class BestFriendsListGui extends GuiScrollingList {
        private final SkyhChatComponent BEST_FRIENDS_MIGRATIONS_GUIDE = new SkyhChatComponent("The 'best friends list' feature has been removed from this mod, as Hypixel added a very similar feature ").red()
                .appendSibling(new SkyhChatComponent("[click to open patch notes]")
                        .darkAqua()
                        .setUrl("https://hypixel.net/threads/social-update-online-status-best-friends-more.4638020/"))
                .appendFreshSibling(new SkyhChatComponent("You can either…\n §e➊ §ftry to add someone from your Skyhaven best friends list to your Hypixel best friends list (§2+§f button), or\n §e➋ §fdiscard/remove them from your Skyhaven best friends list without trying to add them on Hypixel as a best friend (§c♺§f button)."
                        + "\n§4§l❢ §eOnly players who are already on your §6normal §eHypixel friends list can be added as a best friends."))
                .appendFreshSibling(new SkyhChatComponent("➜ Start migrating my clicking the button at the top.").gray());
        ...
        public BestFriendsListGui(int width, int height, boolean isMigrating) {
            super(SkyhConfigBestFriendsMigration.this.mc,
                    width - 6, height,
                    27, height - 3, width + 3, 18,
                    SkyhConfigBestFriendsMigration.this.width, SkyhConfigBestFriendsMigration.this.height);
        ...
        private List<BestFriendEntry> convertToEntries() {
            Set<String> bestFriends = Skyhaven.getInstance().getFriendsHandler().getBestFriends();
        ...
        protected void drawHeader(int entryRight, int relativeY, Tessellator tess) {
            ...
            if (hasEntriesLeft) {
                SkyhConfigBestFriendsMigration.this.fontRendererObj.drawStringWithShadow(EnumChatFormatting.GREEN + "Migrate " + EnumChatFormatting.WHITE + "or " + EnumChatFormatting.RED + "remove " + EnumChatFormatting.WHITE + "best friend", this.left + 6, relativeY + 5, 0xFFCC00);
        ...
        protected void clickHeader(int x, int y) {
            ...
            if (this.startMigratingButton.isMouseOver()) {
                SkyhConfigBestFriendsMigration.this.isMigrating = true;
                SkyhConfigBestFriendsMigration.this.initGui();
            }
        }
        ...
        private List<IChatComponent> resizeContent() {
            return new ArrayList<>(GuiUtilRenderComponents.splitText(BEST_FRIENDS_MIGRATIONS_GUIDE,
                    this.listWidth - 8, SkyhConfigBestFriendsMigration.this.fontRendererObj, false, true));
        }
            ...
            if (isMigrating) {
                ...
                if (line != null) {
                    ...
                    for (IChatComponent part : line) {
                        ...
                        xOffset += SkyhConfigBestFriendsMigration.this.fontRendererObj.getStringWidth(((ChatComponentText) part).getChatComponentText_TextValue());
                        if (xOffset >= this.mouseX) {
                            SkyhConfigBestFriendsMigration.this.handleComponentClick(part);
                            break;
                        }
                    }
                }
        ...
        protected void drawSlot(int slotIdx, int entryRight, int slotTop, int slotBuffer, Tessellator tess) {
            if (this.isMigrating) {
                ...
                if (line != null) {
                    SkyhConfigBestFriendsMigration.this.fontRendererObj.drawStringWithShadow(line.getFormattedText(), this.left + 4, slotTop, 0xFFffffff);
                }
            }
        }
        ...
        private class BestFriendEntry {
            ...
            public BestFriendEntry(String bestFriend) {
                ...
                this.btnRemoveTooltip.add("§cRemove §e" + bestFriend + " §ffrom your Skyhaven best friends list");
                this.btnRemoveTooltip.add("§7Runs the command §8/skyh remove " + bestFriend);
            }
            ...
            public void drawEntry(int x, int y, int right) {
                ...
                SkyhConfigBestFriendsMigration.this.fontRendererObj.drawStringWithShadow(this.bestFriend, (currentX + 1), (y + 4), friendStatus);
            ...
            public void mousePressed(int mouseX, int mouseY, int slotIndex) {
                if (btnAdd.mousePressed(mc, mouseX, mouseY)) {
                    ...
                    if (isShiftKeyDown()) {
                        ...
                        GuiYesNo guiDeleteRule = new GuiYesNo(SkyhConfigBestFriendsMigration.this,
                                EnumChatFormatting.RED + "Discard/remove " + EnumChatFormatting.YELLOW + bestFriend + EnumChatFormatting.RESET + " from Skyhaven best friends list?",
            ...
            public void removeBestFriend() {
                Skyhaven.getInstance().getFriendsHandler().removeBestFriend(this.bestFriend);
