...
public class SkyhConfigPreview {
    ...
    public static SkyhConfigGui parent;

    public SkyhConfigPreview(IChatComponent chatComponent) {
        this.type = Type.CHAT;
        this.chatComponent = chatComponent;
    }

    public SkyhConfigPreview(ItemStack... items) {
        this.type = Type.ITEM;
        this.items = items;
    }

    public SkyhConfigPreview(PreviewChatRunnable previewRunnable) {
        this.type = Type.RUNNABLE;
        this.previewRunnable = previewRunnable;
    }

    public SkyhConfigPreview(String sound, float volume, float pitch) {
    ...
    public SkyhConfigPreview(DataHelper.DungeonClass dungeonClass) {
        this(new SkyhConfigPreview.PreviewChatRunnable() {
            @Override
            public IChatComponent getText() {
                String markWith;
                switch (SkyhConfig.dungeonPartyMarker(dungeonClass)) {
                    case UNIDEAL_DUPE:
                ...
                return new SkyhChatComponent("Marked with: " + markWith).gray();
            }
        });
    }
    ...
    public static IChatComponent createDemoOnline(String name, String gameMode, String onlineTime) {
        return new SkyhChatComponent(name).darkGreen().setHover(new SkyhChatComponent(gameMode).yellow().appendFreshSibling(new SkyhChatComponent("Online for " + onlineTime).white()));
    }
