package de.skyh.skyhaven.search;
...
public class GuiSearch extends GuiScreen {
    ...
    private final ExecutorService executorService = new ThreadPoolExecutor(0, 1,
            ...
            new LinkedBlockingQueue<>(), new ThreadFactoryBuilder().setNameFormat(Skyhaven.MODID + "-logfilesearcher-%d").build());
    // data
    ...
    public GuiSearch(String initialSearchQuery) {
        this.mcLogOutputFile = new File(Skyhaven.getInstance().getModOutDirectory(), "skyhaven-mc-log-search-temp.txt");
        try {
        ...
        this.dateStart = SkyhConfig.calculateStartDate();
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        if (button == this.buttonSearch) {
            ...
            this.searchResults.add(new LogEntry("" + EnumChatFormatting.GOLD + EnumChatFormatting.BOLD + "Performing a search " + EnumChatFormatting.GRAY + EnumChatFormatting.ITALIC + "/skyh search [initial search term]"));
            ...
            for (Property configEntry : Skyhaven.getInstance().getConfig().getLogSearchProperties()) {
                logSearchConfigElements.add(new ConfigElement(configEntry));
            }
            mc.displayGuiScreen(new GuiConfig(this,
                    logSearchConfigElements,
                    Skyhaven.MODID, "skyhavenLogSearchConfig", false, false,
