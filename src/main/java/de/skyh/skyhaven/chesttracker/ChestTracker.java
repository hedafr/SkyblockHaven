package de.skyh.skyhaven.chesttracker;

import de.skyh.skyhaven.Skyhaven;
import de.skyh.skyhaven.chesttracker.data.HyBazaarData;
import de.skyh.skyhaven.chesttracker.data.HyItemsData;
import de.skyh.skyhaven.chesttracker.data.ItemData;
import de.skyh.skyhaven.chesttracker.data.LowestBinsCache;
...
public class ChestTracker {
    ...
    public ChestTracker(Skyhaven main) {
        this.main = main;
        refreshPriceCache();
        ...
        if (allowUpdateBazaar()) {
            ...
            ApiUtils.fetchBazaarData(bazaarData -> {
                if (bazaarData == null || !bazaarData.isSuccess()) {
                    main.getChatHelper().sendMessage(new SkyhChatComponent("Error: Couldn't get Bazaar data from Hypixel API! API might be down: check status.hypixel.net").red().setUrl("https://status.hypixel.net/"));
                }
        ...
        if (allowUpdateLowestBins()) {
            ...
            ApiUtils.fetchLowestBins(lowestBins -> {
                if (!lowestBins.hasData()) {
                    main.getChatHelper().sendMessage(new SkyhChatComponent("Error: Couldn't get lowest BINs from Moulberry's API! API might be down: check if " + ApiUtils.LOWEST_BINS + " is reachable.").red().setUrl(ApiUtils.LOWEST_BINS));
                }
        ...
        if (allowUpdateNpcSell()) {
            ...
            ApiUtils.fetchItemsData(itemsData -> {
                ...
                if (itemsData == null || !itemsData.isSuccess()) {
                    main.getChatHelper().sendMessage(new SkyhChatComponent("Error: Couldn't get Items data from Hypixel API! API might be down: check status.hypixel.net").red().setUrl("https://status.hypixel.net/"));
                } else {
    ...
    public void markChestsWithWantedItem(String sbKey, int amount, String itemName) {
        ...
        final SkyhChatComponent relevantChestCoordsHover = new SkyhChatComponent("Chests with ").gold().bold().appendSibling(new SkyhChatComponent(itemName).reset())
                .appendFreshSibling(new SkyhChatComponent(StringUtils.repeat(' ', maxItemCountLength) + "        (x | y | z)").gray());
        ...
        chestsWithWantedItemsAndCount.entrySet().stream()
                ...
                .forEach(wantedChest -> {
                    ...
                    relevantChestCoordsHover.appendFreshSibling(new SkyhChatComponent(" " + EnumChatFormatting.YELLOW + itemCountInChest + EnumChatFormatting.DARK_GRAY + "x ➜ "
        ...
        if (relevantChestCount > relevantChestsCoordsLimit) {
            relevantChestCoordsHover.appendFreshSibling(new SkyhChatComponent("      + " + (relevantChestCount - relevantChestsCoordsLimit) + " more chests").gray());
        }
        main.getChatHelper().sendMessage(new SkyhChatComponent("Chest Tracker & Analyzer is now highlighting " + EnumChatFormatting.LIGHT_PURPLE + relevantChestCount + EnumChatFormatting.GREEN + " chest" + (relevantChestCount > 1 ? "s" : "") + " with " + itemName
                + EnumChatFormatting.DARK_GREEN + " [hover for coords, click to re-open GUI]").green()
                .setHover(relevantChestCoordsHover)
                .setSuggestCommand("/skyh analyzeChests", false));
