...
public class DungeonsPartyListener {
    ...
    public DungeonsPartyListener(Skyhaven main) {
    ...
    public DungeonsPartyListener(Skyhaven main, List<String> partyMembers) {
        if (nextStep == Step.STOP) { // prevent double-registration of this listener
            this.main = main;
    ...
    private void sendDungeonPartyStats() {
        ...
        SkyhChatComponent dungeonsParty = new SkyhChatComponent("Dungeons party").gold().bold();
        ...
        for (Map.Entry<String, Optional<UUID>> partyMember : partyMembers.entrySet()) {
            if (playerEntry.length() > 0) {
                ...
                SkyhChatComponent dungeonPartyEntry = new SkyhChatComponent(playerEntry.toString())
                        .setSuggestCommand((partyMemberName.equals(thePlayerName) ? "/boop " : "/p kick ") + partyMemberName, false);
                if (playerTooltip.length() > 0) {
                    dungeonPartyEntry.setHover(new SkyhChatComponent(playerTooltip.toString()));
                }
            ...
            if (sbStats != null && sbStats.isSuccess()) {
                ...
                if (selectedClass != null) {
                    ...
                    classAndDungeonTypeInfo = selectedClass.getName() + " " + (SkyhConfig.useRomanNumerals() ? Utils.convertArabicToRoman(selectedClassLevel) : selectedClassLevel);
                }
