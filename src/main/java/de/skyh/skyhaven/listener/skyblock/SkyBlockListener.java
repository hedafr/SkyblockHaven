package de.skyh.skyhaven.listener.skyblock;
...
public class SkyBlockListener {
    ...
    public SkyBlockListener(Skyhaven main) {
        this.main = main;
    ...
    public void onItemLookupInGui(GuiScreenEvent.KeyboardInputEvent.Pre e) {
        ...
        if (SkyhConfig.isLookupWikiKeyBindingPressed()) {
            itemLookupType = ItemLookupType.WIKI;
        } else if (SkyhConfig.isLookupOfficialWikiKeyBindingPressed()) {
            itemLookupType = ItemLookupType.OFFICIAL_WIKI;
        } else if (SkyhConfig.isLookupPriceKeyBindingPressed()) {
        ...
        if (itemLookupType != ItemLookupType.INVALID && Keyboard.getEventKeyState() && e.gui instanceof GuiContainer) {
            ...
            if (hoveredSlot != null && hoveredSlot.getHasStack()) {
                ...
                if (itemBaseName != null) {
                    ...
                    main.getChatHelper().sendMessage(new SkyhChatComponent(EnumChatFormatting.DARK_GREEN + " ➡ "
                            + EnumChatFormatting.GREEN + "Open" + (SkyhConfig.lookupItemDirectly ? "ing " : " ") + itemLookupType.getDescription() + " for " + itemBaseName).green()
                            .setUrl(link, itemLookupType.description + ": " + EnumChatFormatting.WHITE + link));

                    if (SkyhConfig.lookupItemDirectly) {
    ...
    public void onItemTooltip(ItemTooltipEvent e) {
        ...
        if ((SkyhConfig.getBazaarConnectGraphsNodes() == SkyhConfig.Setting.ALWAYS
                || SkyhConfig.getBazaarConnectGraphsNodes() == SkyhConfig.Setting.SPECIAL && SkyhConfig.isTooltipToggleKeyBindingPressed())
            ...
            } else if (currentScreen instanceof SkyhConfigGui) {
        ...
        if ((SkyhConfig.getTooltipPetExpDisplay() == SkyhConfig.Setting.ALWAYS
                || SkyhConfig.getTooltipPetExpDisplay() == SkyhConfig.Setting.SPECIAL && SkyhConfig.isTooltipToggleKeyBindingPressed())
        ...
        SkyhConfig.Setting tooltipItemAgeDisplay = SkyhConfig.getTooltipItemAgeDisplay();
        SkyhConfig.Setting tooltipItemTimestampDisplay = SkyhConfig.getTooltipItemTimestampDisplay();

        // add item age to tooltip
        if (extraAttributes != null && extraAttributes.hasKey("timestamp")
                && (tooltipItemAgeDisplay != SkyhConfig.Setting.DISABLED || tooltipItemTimestampDisplay != SkyhConfig.Setting.DISABLED)) {
            ...
            if (localZoneDateTime != null) {
                ...
                switch (tooltipItemTimestampDisplay) {
                    case SPECIAL:
                        if (!SkyhConfig.isTooltipToggleKeyBindingPressed()) {
                            break;
                        }
                    case ALWAYS:
                ...
                switch (tooltipItemAgeDisplay) {
                    ...
                    case ALWAYS: {
                        ...
                        String itemAge = itemAgeInMs >= 60_000
                                ? (SkyhConfig.tooltipItemAgeShortened ? Utils.getDurationAsWord(itemCreationTimestamp) : Utils.getDurationAsWords(itemCreationTimestamp).first())
                                : "<1 minute";
        ...
        // bestiary overview:
        String bestiaryOverviewOrder = SkyhConfig.bestiaryOverviewOrder;
        ...
        if (currentScreen instanceof GuiChest && isBestiaryOverviewVisible && isBestiaryMainGui((GuiChest) currentScreen, e.itemStack)) {
            e.toolTip.add("");
            e.toolTip.add("" + EnumChatFormatting.GOLD + EnumChatFormatting.BOLD + EnumChatFormatting.UNDERLINE + "Skyhaven: Bestiary overview");
            ...
            e.toolTip.add(EnumChatFormatting.DARK_GRAY + " & Skyhaven tooltip shows kills " + EnumChatFormatting.ITALIC + "left" + EnumChatFormatting.RESET + EnumChatFormatting.DARK_GRAY + "!)");
        ...
        } else if (currentScreen instanceof SkyhConfigGui && isBestiaryOverviewVisible && e.itemStack.getItem() == Items.wheat) {
            if (isDifferentTriggerItem) {
                ...
                bestiaryOverview.add(new BestiaryEntry(EnumChatFormatting.GREEN + "Skyh", true));
            }
        } else {
        ...
        if ((e.itemStack.getItem() == Items.cauldron || e.itemStack.getItem() == Item.getItemFromBlock(Blocks.chest)) && !"unordered".equals(SkyhConfig.bazaarSellAllOrder)) {
            ...
            if ("Sell Inventory Now".equals(hoveredItemName) || "Sell Sacks Now".equals(hoveredItemName)) {
                ...
                if ("high → low".equals(SkyhConfig.bazaarSellAllOrderAscDesc)) {
                ...
                for (int i = 0; i < toolTip.size(); i++) {
                    ...
                    if (bazaarSellMatcher.matches()) {
                        ...
                        try {
                            double key;
                            String suffix = "";
                            switch (SkyhConfig.bazaarSellAllOrder) {
                                case "price (sum)":
                ...
                if (sellEntries.size() > 1) {
                    ...
                    e.toolTip.add(startIndex + sellEntryIndex, EnumChatFormatting.DARK_GRAY + "  » ordered by " + SkyhConfig.bazaarSellAllOrder);
                }
            }
        }

        // bazaar: show how many items left on offer/order
        if (SkyhConfig.bazaarShowItemsLeft) {
        ...
        SkyhConfig.Setting tooltipAuctionHousePriceEachDisplay = SkyhConfig.getTooltipAuctionHousePriceEachDisplay();
        if ((tooltipAuctionHousePriceEachDisplay == SkyhConfig.Setting.ALWAYS || tooltipAuctionHousePriceEachDisplay == SkyhConfig.Setting.SPECIAL && SkyhConfig.isTooltipToggleKeyBindingPressed())
                && (e.entityPlayer.openContainer instanceof ContainerChest || currentScreen instanceof SkyhConfigGui)) {
                ...
                if (toolTipLineUnformatted.startsWith("Top bid: ")
                    ...
                    try {
                        ...
                        if (enchantmentName != null) {
                            toolTip.add(i + 1, EnumChatFormatting.YELLOW + "  (≙ " + itemAmount + "x " + EnumChatFormatting.BLUE + enchantmentName + " " + (SkyhConfig.useRomanNumerals() ? "I" : "1") + EnumChatFormatting.RESET + EnumChatFormatting.YELLOW + " for " + formattedPriceEach + " each)");
                        } else {
    ...
    public void onRenderGuiBackground(GuiScreenEvent.DrawScreenEvent.Pre e) {
        if (e.gui instanceof GuiChest) {
            SkyhConfig.Setting markAuctionHouseEndedAuctions = SkyhConfig.getMarkAuctionHouseEndedAuctions();
            if (markAuctionHouseEndedAuctions == SkyhConfig.Setting.DISABLED) {
                return;
            }
                        ...
                        if (itemNbtDisplay != null && itemNbtDisplay.hasKey("Lore", Constants.NBT.TAG_LIST)) {
                            ...
                            for (int loreLineNr = loreList.tagCount() - 1; loreLineNr >= 0; --loreLineNr) {
                                ...
                                if (loreLine.startsWith("Ends in: ")) {
                                    ...
                                    auctionStatus = EnumChatFormatting.RED + (markAuctionHouseEndedAuctions == SkyhConfig.Setting.TEXT ? "Expired" : "E");
                                } else if (loreLine.equals("Status: Ended!")) {
                                    auctionStatus = EnumChatFormatting.GREEN + (markAuctionHouseEndedAuctions == SkyhConfig.Setting.TEXT ? "Ended" : "E");
                                } else if (loreLine.equals("Status: Sold!")) {
                                    auctionStatus = EnumChatFormatting.GREEN + (markAuctionHouseEndedAuctions == SkyhConfig.Setting.TEXT ? "Sold" : "S");
                                } else {
                                    continue;
                                }
                                GlStateManager.pushMatrix();
                                GlStateManager.translate(0, 0, 281);
                                double scaleFactor = markAuctionHouseEndedAuctions == SkyhConfig.Setting.TEXT ? 0.4 : 0.5;
    ...
    private Pair<String, Integer> extractEnchantedBookInfo(GuiScreen currentScreen, ItemStack currentItem) {
        ...
        if (!shouldDisplayPriceEach && inventoryName.startsWith("Enchantments ➜ ")) {
            if (currentItemName.contains("Turbo-")) {
                ...
                for (String priceEachEnchantment : SkyhConfig.tooltipAuctionHousePriceEachEnchantments) {
