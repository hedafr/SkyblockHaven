...
import de.skyh.skyhaven.data.DataHelper.DungeonClass;
...
public class DungeonsListener {
    ...
    public DungeonsListener(Skyhaven main) {
        this.main = main;
    ...
    public void onItemTooltip(ItemTooltipEvent e) {
        ...
        boolean toggleKeyBindingPressed = SkyhConfig.isDungeonItemTooltipToggleKeyBindingPressed() && !main.getDungeonCache().isInDungeon();
        SkyhConfig.Setting showItemQualityAndFloorDisplay = SkyhConfig.getShowItemQualityAndFloorDisplay();
        boolean showItemQualityAndFloor = showItemQualityAndFloorDisplay == SkyhConfig.Setting.ALWAYS
                || (showItemQualityAndFloorDisplay == SkyhConfig.Setting.SPECIAL && toggleKeyBindingPressed);
                ...
                if (lineMatcher.matches()) {
                    if (EnumChatFormatting.getTextWithoutFormattingCodes(lineMatcher.group("prefix")).equals("Gear Score: ")) {
                        ...
                        if (!hasCustomGearScore) {
                            if (SkyhConfig.dungItemQualityShortenNonRandomized) {
                        ...
                        if (showItemQualityAndFloor) {
                            if (SkyhConfig.isDungItemQualityAtTop()) {
                                if (SkyhConfig.dungItemHideGearScore) {
    ...
    private void renderPartyStatus(ItemStack item, int x, int y) {
        ...
        if (lastToolTipLine.endsWith("Complete previous floor first!")
            ...
            if (SkyhConfig.dungeonPartyMarker(activeDungeonClass).ifDuped()) {
            ...
            for (int i = 0; i < itemTooltip.size(); i++) {
                ...
                if (playerDetailMatcher.matches()) {
                    ...
                    if (SkyhConfig.dungeonPartyMarker(clazz) == SkyhConfig.Mark.BLOCK_ALWAYS) {
                    ...
                    if (classLevel < SkyhConfig.dungClassMin) {
                    ...
                    if (minDungLevelReq < SkyhConfig.dungDungeonReqMin) {
            ...
            if (partyType != DataHelper.PartyType.UNJOINABLE_OR_BLOCK) {
                ...
                for (Map.Entry<DungeonClass, AtomicInteger> partyClassInfo : dungClassesInParty.entrySet()) {
                    SkyhConfig.Mark dungeonPartyMarker = SkyhConfig.dungeonPartyMarker(partyClassInfo.getKey());
                    if (dungeonPartyMarker == SkyhConfig.Mark.UNIDEAL_DUPE && partyClassInfo.getValue().get() > 1) {
                        // 2+ class
                        dupedClasses.append(partyClassInfo.getKey().getShortName());
                    } else if (dungeonPartyMarker == SkyhConfig.Mark.UNIDEAL_ALWAYS) {
                        unwantedClasses.append(partyClassInfo.getKey().getShortName());
                    } else if (dungeonPartyMarker == SkyhConfig.Mark.BLOCK_DUPE && partyClassInfo.getValue().get() > 1) {
                ...
                if (partyType != DataHelper.PartyType.UNJOINABLE_OR_BLOCK) {
                    ...
                    if (SkyhConfig.dungPartiesSize && partySize > 0) {
                        GlStateManager.pushMatrix();
                        GlStateManager.translate(0, 0, 280);
        ...
        if (SkyhConfig.dungPartyFinderOverlayDrawBackground &&
                (partyType != DataHelper.PartyType.CURRENT
    ...
    public void onGuiInit(GuiScreenEvent.InitGuiEvent.Post e) {
        if (SkyhConfig.dungPartyFinderRuleEditorShowOpenButton && e.gui instanceof GuiChest) {
    ...
    public void onGuiButtonClick(GuiScreenEvent.ActionPerformedEvent.Post e) {
        if (SkyhConfig.dungPartyFinderRuleEditorShowOpenButton && e.gui instanceof GuiChest && e.button.id == 8765 && "Rule editor ↗".equals(e.button.displayString)) {
    ...
    public void onGuiButtonHover(GuiScreenEvent.DrawScreenEvent.Post e) {
        if (SkyhConfig.dungPartyFinderRuleEditorShowOpenButton && e.gui instanceof GuiChest && btnOpenRuleEditor != null && btnOpenRuleEditor.isMouseOver()) {
            Minecraft mc = Minecraft.getMinecraft();
            List<String> btnTooltip = new ArrayList<>();
            btnTooltip.add("Open Skyhaven's Party Notes Rule Editor");
            btnTooltip.add(EnumChatFormatting.GRAY + "Want to remove this button? " + EnumChatFormatting.YELLOW + "/skyh config editor");
            btnTooltip.add(EnumChatFormatting.GRAY + "Want to remove the overlay inside dungeons? " + EnumChatFormatting.YELLOW + "/skyh config overlay");
    ...
    public void onMessageReceived(ClientChatReceivedEvent e) {
        if (e.type != 2) { // normal chat or system msg (not above action bar)
            ...
            if (!main.getDungeonCache().isInDungeon()) {
                ...
                } else if (SkyhConfig.dungSendWrongFloorWarning) {
                    ...
                    if (dungeonEnteredMatcher.matches()) {
                        ...
                        if (queuedFloor != null && !queuedFloor.equals(floor)) {
                            ...
                            new TickDelay(() -> {
                                ...
                                Minecraft.getMinecraft().thePlayer.playSound("mob.skyh.hurt", Minecraft.getMinecraft().gameSettings.getSoundLevel(SoundCategory.MASTER), 1);
                            }, 100);
                        }
                    }
                }
            ...
            if (dungeonDeathMatcher.matches()) {
                ...
                // dungeon "end screen"
                if (SkyhConfig.dungSendPerformanceOnEndScreen) {
        ...
        if (e.gui instanceof GuiChest) {
            ...
            if (inventory.getName().equals("Party Finder")) {
                ...
                if (hoveredSlot != null && hoveredSlot.getHasStack()) {
                    ...
                    if (CredentialStorage.isSkyhValid && SkyhConfig.dungPartyFinderPartyLookup) {
    ...
    public void onRenderGameOverlay(RenderGameOverlayEvent.Post e) {
        if (e.type == RenderGameOverlayEvent.ElementType.ALL) {
            ...
            boolean isEditingDungeonOverlaySettings = SkyhConfigGui.showDungeonPerformanceOverlay();
            if (SkyhConfig.dungOverlayEnabled && dungeonCache.isInDungeon() || isEditingDungeonOverlaySettings) {
                ...
                if (!SkyhConfig.hasOpenedConfigGui) {
                    dungeonPerformanceEntries.add(EnumChatFormatting.RED + " ⚠ Want to move me? " + EnumChatFormatting.DARK_GRAY + "➡ " + EnumChatFormatting.LIGHT_PURPLE + "/skyh config overlay");
                }

                FontRenderer fontRenderer = Minecraft.getMinecraft().fontRendererObj;
                GlStateManager.pushMatrix();

                float scaleFactor = SkyhConfig.dungOverlayGuiScale / 100f;
                GlStateManager.scale(scaleFactor, scaleFactor, 0);
                for (int line = 0; line < dungeonPerformanceEntries.size(); line++) {
                    String dungeonPerformanceEntry = dungeonPerformanceEntries.get(line);
                    int xPos = (int) ((e.resolution.getScaledWidth() * (SkyhConfig.dungOverlayPositionX / 1000d)) / scaleFactor);
                    int yPos = (int) ((e.resolution.getScaledHeight() * (SkyhConfig.dungOverlayPositionY / 1000d) + 2) / scaleFactor + fontRenderer.FONT_HEIGHT * line + 2);
                    SkyhConfig.Setting dungOverlayTextBorder = SkyhConfig.getDungOverlayTextBorder();
                    if (dungOverlayTextBorder == SkyhConfig.Setting.TEXT) {
                        // normal mc text shadow (drop shadow)
                        fontRenderer.drawStringWithShadow(dungeonPerformanceEntry, xPos, yPos, 0xffFFAA00);
                    } else if (dungOverlayTextBorder == SkyhConfig.Setting.ALWAYS) {
                        // full outline
