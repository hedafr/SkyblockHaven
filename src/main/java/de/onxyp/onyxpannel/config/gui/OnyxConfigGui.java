 ...
 * <ul>
 * <li>menu ({@link OnyxConfigMenuList}) with list of config categories ({@link OnyxConfigCategory})</li>
 * <li>the current opened config category with its sub-categories ({@link OnyxConfigCategoryScrolling})</li>
 * </ul>
 * Based on {@link net.minecraft.client.gui.GuiControls}
 */
public class OnyxConfigGui extends GuiScreen {
    ...
    private OnyxConfigMenuList menu;
    private int selectedMenuIndex = -1;
    private OnyxConfigCategory currentConfigCategory;
    private final boolean isOutsideOfSkyBlock;
    /**
     * equivalent of GuiModList.keyBindingList
     */
    private OnyxConfigCategoryScrolling currentConfigCategoryGui;
    private GuiButton btnClose;
    private GuiTextField fieldSearchQuery;
    private String searchQuery;

    public OnyxConfigGui(String searchQuery) {
    ...
    public void initGui() {
        ...
        this.menu = new OnyxConfigMenuList(this, menuWidth);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        String guiTitle = "" + EnumChatFormatting.BOLD + EnumChatFormatting.UNDERLINE + Onyxpannel.MODNAME + " config:" + EnumChatFormatting.RESET + " " + (currentConfigCategory != null ? currentConfigCategory.getDisplayName() : "Search");
    ...
    public void drawDefaultBackground() {
        if (!OnyxConfigGui.showDungeonPerformanceOverlay()) {
    ...
    public boolean selectConfigCategory(int index) {
        ...
        if (index >= 0 && index <= OnyxConfig.getConfigCategories().size()) {
            this.currentConfigCategory = OnyxConfig.getConfigCategories().get(selectedMenuIndex);
        ...
        Onyxpannel.getInstance().getConfig().syncFromGui();
        return true;
    }
    ...
    public void switchDisplayedConfigCategory() {
        if (currentConfigCategory == null) {
            // display search
            currentConfigCategoryGui = new OnyxConfigCategoryScrolling(this, mc, searchQuery, menu.getRight() + 3);
        } else {
            // display one category
            currentConfigCategoryGui = new OnyxConfigCategoryScrolling(this, mc, currentConfigCategory, menu.getRight() + 3);
        }
    }
    ...
    public void onGuiClosed() {
        ...
        if (OnyxConfigPreview.parent != null) {
            OnyxConfigPreview.parent = null;
        }
    }
