 ...
 */
public class OnyxConfigCategoryScrolling extends GuiListExtended {
    private final Minecraft mc;
    private final List<IGuiListEntry> listEntries;
    private final Map<Integer, List<String>> explanations;
    private final OnyxConfigGui parent;
    private int maxListLabelWidth = 0;
    private KeyBindingConfigEntry currentKeyBindingConfigEntry = null;
    /**
     * listEntryIndex => (sub-category) preview
     */
    private final NavigableMap<Integer, OnyxConfigPreview> listEntriesPreviews;
    private final Set<String> newConfigOptions;

    private OnyxConfigCategoryScrolling(OnyxConfigGui parent, Minecraft mc, int marginLeft) {
    ...
    public OnyxConfigCategoryScrolling(OnyxConfigGui parent, Minecraft mc, String initialSearchQuery, int marginLeft) {
        this(parent, mc, marginLeft);
        showFilteredConfigEntries(initialSearchQuery);
    }

    public OnyxConfigCategoryScrolling(OnyxConfigGui parent, Minecraft mc, OnyxConfigCategory currentConfigCategory, int marginLeft) {
        this(parent, mc, marginLeft);

        for (OnyxConfigCategory.SubCategory subCategory : currentConfigCategory.getSubCategories()) {
            int subCategoryStartIndex = this.listEntries.size();
            this.listEntries.add(new OnyxConfigCategoryScrolling.CategoryEntry(subCategory.getDisplayName(), !subCategory.getExplanations().isEmpty()));

            explanations.put(subCategoryStartIndex, subCategory.getExplanations());
            int explanationsLineCount = 0;
            if (subCategory.getExplanations().size() > 0 && OnyxConfig.getConfigGuiExplanationsDisplay() == OnyxConfig.Setting.TEXT) {
            ...
            if ("Other settings".equals(subCategory.getDisplayName())) {
                this.listEntries.add(new GuiSwitchEntry("gotoKeyBindings", "Controls ↗", () -> mc.displayGuiScreen(new GuiControls(OnyxConfigCategoryScrolling.this.parent, mc.gameSettings))));
            ...
            // add previews
            Map<Integer, OnyxConfigPreview> previews = subCategory.getPreviews();
            this.listEntriesPreviews.put(subCategoryStartIndex, null);
            // previews for specific properties
            for (Map.Entry<Integer, OnyxConfigPreview> previewEntry : previews.entrySet()) {
    ...
    private void addConfigEntryToGui(OnyxConfigCategory.SubCategory subCategory, Property configEntry) {
        ...
        if (configEntry.isList() && type == Property.Type.STRING && configEntry.equals(Onyxpannel.getInstance().getConfig().getTabCompletableNamesCommandsProperty())) {
            this.listEntries.add(new GuiSwitchEntry("tabCompletableNamesCommands", "➡ modify", () ->
                    mc.displayGuiScreen(new GuiConfig(OnyxConfigCategoryScrolling.this.parent,
                            Lists.newArrayList(new ConfigElement(Onyxpannel.getInstance().getConfig().getTabCompletableNamesCommandsProperty())),
                            Onyxpannel.MODID, "onyxpannelTabCompletableCommands", false, false,
            ...
            if (configEntry.getLanguageKey().equals(Onyxpannel.MODID + ".config.isOnyxValid")) {
                // special case: onyx!
                this.listEntries.add(new BooleanConfigEntry(configEntry));
                return;
            } else if (configEntry.equals(Onyxpannel.getInstance().getConfig().getOnyxCmdAliasProperty())) {
                this.listEntries.add(new GuiSwitchEntry("onyxCmdAlias", "➡ modify", () ->
                        mc.displayGuiScreen(new GuiConfig(OnyxConfigCategoryScrolling.this.parent,
                                Lists.newArrayList(new ConfigElement(Onyxpannel.getInstance().getConfig().getOnyxCmdAliasProperty())),
                                Onyxpannel.MODID, "onyxpannelOnyxCmdAlias", false, false,
                                EnumChatFormatting.GOLD + "Press Done to save changes. " + EnumChatFormatting.RED + "Requires a game restart to take effect!"))));
                return;
            } else if (configEntry.equals(Onyxpannel.getInstance().getConfig().getTooltipAuctionHousePriceEachEnchantmentsProperty())) {
                this.listEntries.add(new GuiSwitchEntry("tooltipAuctionHousePriceEachEnchantments", "➡ modify", () ->
                        mc.displayGuiScreen(new GuiConfig(OnyxConfigCategoryScrolling.this.parent,
                                Lists.newArrayList(new ConfigElement(Onyxpannel.getInstance().getConfig().getTooltipAuctionHousePriceEachEnchantmentsProperty())),
                                Onyxpannel.MODID, "onyxpannelTooltipAuctionHousePriceEachEnchantments", false, false,
    ...
    public void showFilteredConfigEntries(String searchQuery) {
        ...
        OnyxConfigCategory lastCategory = null;
        OnyxConfigCategory.SubCategory lastSubCategory = null;
        boolean hasLogSearchBeenAdded = false;
        int entryNr = 0;
        boolean showNewConfigEntries = "new".equalsIgnoreCase(searchQuery);
        for (OnyxConfigCategory configCategory : OnyxConfig.getConfigCategories()) {
            for (OnyxConfigCategory.SubCategory subCategory : configCategory.getSubCategories()) {
                // add config elements
                for (Property configEntry : subCategory.getConfigEntries()) {
                    ...
                    if ((showNewConfigEntries && newConfigOptions.contains(configEntry.getName())) ||
                        ...
                        if (configCategory != lastCategory) {
                            this.listEntries.add(new OnyxConfigCategoryScrolling.CategoryEntry("" + EnumChatFormatting.RESET + EnumChatFormatting.DARK_GRAY + "Config category: " + EnumChatFormatting.DARK_RED + EnumChatFormatting.BOLD + EnumChatFormatting.UNDERLINE + configCategory.getDisplayName(), false));
                        ...
                        // add control buttons to navigate to other guis
                        if ("Other settings".equals(subCategory.getDisplayName())) {
                            if (!hasLogSearchBeenAdded && Onyxpannel.getInstance().getConfig().getLogSearchProperties().contains(configEntry)) {
                                // don't add properties to main config gui, use this instead:
                                int labelWidth = mc.fontRendererObj.getStringWidth(I18n.format("onyxpannel.config.gotoLogSearchConfig"));
                                if (labelWidth > this.maxListLabelWidth) {
                                    this.maxListLabelWidth = labelWidth;
                                }
                            ...
                            if ("onyxpannel.config.dungPartyFinderRuleEditorSimplified".equals(configEntry.getLanguageKey())) {
                                // add rule editor button to 'Use simplified editor' entry
                                this.listEntries.add(new GuiSwitchEntry("gotoPartyFinderRulesEditor", "Rule editor ↗", () -> mc.displayGuiScreen(new RuleEditorGui())));
                            }
                            addConfigEntryToGui(subCategory, configEntry);
                            // add preview for this entry
                            OnyxConfigPreview preview = subCategory.getPreview(configEntry);
                            if (preview != null) {
    ...
    protected void drawSlot(int slotIndex, int x, int y, int slotHeight, int mouseXIn, int mouseYIn) {
        if (y >= this.top - 5 && y <= this.bottom) {
            ...
            OnyxConfigPreview preview = listEntriesPreviews.get(slotIndex);
    ...
    public void drawScreen(int mouseX, int mouseY, float p_148128_3_) {
        if (this.field_178041_q) {
            ...
            if (hoveredSlotIndex >= 0) {
                ...
                OnyxConfig.Setting configGuiExplanationsDisplay = OnyxConfig.getConfigGuiExplanationsDisplay();
                if (explanations != null && !explanations.isEmpty() && (configGuiExplanationsDisplay == OnyxConfig.Setting.TOOLTIP || configGuiExplanationsDisplay == OnyxConfig.Setting.SPECIAL)) {
                    if (configGuiExplanationsDisplay == OnyxConfig.Setting.SPECIAL) {
                ...
                if (hoveredEntry instanceof BaseConfigEntry) {
                    ...
                    OnyxConfigPreview.drawPreviewHover(mouseX, mouseY);
                }
            }
        }
    }
    ...
    public class CategoryEntry implements GuiListExtended.IGuiListEntry {
        ...
        public CategoryEntry(String labelText, boolean hasExplanations) {
            ...
            this.labelWidth = OnyxConfigCategoryScrolling.this.mc.fontRendererObj.getStringWidth(this.labelText);
            this.hasExplanations = hasExplanations;
        }

        public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected) {
            GuiHelper.drawThinHorizontalLine(x, OnyxConfigCategoryScrolling.this.right, y + (slotHeight - OnyxConfigCategoryScrolling.this.mc.fontRendererObj.FONT_HEIGHT) / 2, 0x99ffffff);

            int yTextPos = y + slotHeight - OnyxConfigCategoryScrolling.this.mc.fontRendererObj.FONT_HEIGHT + 3;
            if (hasExplanations && OnyxConfig.getConfigGuiExplanationsDisplay() != OnyxConfig.Setting.DISABLED) {
                // draw "!" to indicate a sub-category has additional info
                OnyxConfigCategoryScrolling.this.mc.fontRendererObj.drawString(EnumChatFormatting.DARK_GREEN + "❢", x + 2, yTextPos, 0xffffff);
            }
            // draw sub category label
            int labelX = Math.max((int) (x + OnyxConfigCategoryScrolling.this.maxListLabelWidth * 0.75 - this.labelWidth / 2), x + (hasExplanations ? 10 : 0));
            OnyxConfigCategoryScrolling.this.mc.fontRendererObj.drawString(this.labelText, labelX, yTextPos, 0xffffff);
        }
    ...
    public class ExplanationsEntry implements IGuiListEntry {
        ...
        @Override
        public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected) {
            FontRenderer fontRendererObj = OnyxConfigCategoryScrolling.this.mc.fontRendererObj;
    ...
    public abstract class BaseConfigEntry implements GuiListExtended.IGuiListEntry {
        ...
        public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected) {
            this.x = x;
            this.y = y;
            OnyxConfigCategoryScrolling.this.mc.fontRendererObj.drawString(this.name, x, y + slotHeight / 2 - OnyxConfigCategoryScrolling.this.mc.fontRendererObj.FONT_HEIGHT / 2 + 2, 16777215);

            this.btnUndoChanges.xPosition = x + OnyxConfigCategoryScrolling.this.maxListLabelWidth + 88;
            this.btnUndoChanges.yPosition = y + 1;
            this.btnUndoChanges.enabled = this.isChanged();
            this.btnUndoChanges.drawButton(OnyxConfigCategoryScrolling.this.mc, mouseX, mouseY);

            this.btnDefault.xPosition = x + OnyxConfigCategoryScrolling.this.maxListLabelWidth + 88 + 20;
            this.btnDefault.yPosition = y + 1;
            this.btnDefault.enabled = !isDefault();
            this.btnDefault.drawButton(OnyxConfigCategoryScrolling.this.mc, mouseX, mouseY);
        }
        ...
        public void mouseReleased(int slotIndex, int x, int y, int mouseEvent, int relativeX, int relativeY) {
            ...
            Onyxpannel.getInstance().getConfig().syncFromGuiWithoutSaving();
        }
        ...
        public void drawTooltip() {
            int mouseX = OnyxConfigCategoryScrolling.this.mouseX;
            int mouseY = OnyxConfigCategoryScrolling.this.mouseY;
            if (mouseX >= x && OnyxConfigCategoryScrolling.this.mouseX < x + maxListLabelWidth && mouseY >= y && mouseY < y + slotHeight) {
                // mouse is over entry
    ...
    public abstract class ButtonConfigEntry extends BaseConfigEntry {
        ...
        private ButtonConfigEntry(Property property) {
            ...
            if (property.getLanguageKey().equals(Onyxpannel.MODID + ".config.isOnyxValid")) {
        ...
        @Override
        public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected) {
            this.btnChangeConfigEntry.xPosition = x + OnyxConfigCategoryScrolling.this.maxListLabelWidth + 5;
            this.btnChangeConfigEntry.yPosition = y;
            this.btnChangeConfigEntry.drawButton(OnyxConfigCategoryScrolling.this.mc, mouseX, mouseY);
    ...
    private class GuiSwitchEntry extends ButtonConfigEntry {
        ...
        private GuiSwitchEntry(String langKey, String buttonText, Runnable runnable) {
            super(new Property("fakeProperty", "fakeValue", Property.Type.STRING, Onyxpannel.MODID + ".config." + langKey));
    ...
    private class DungCycleConfigEntry extends CycleConfigEntry {
        ...
        protected void changeConfigEntryButtonPressed() {
            ...
            OnyxConfigGui.showDungeonPerformanceOverlayUntil = System.currentTimeMillis() + 3000;
        }
    }
    ...
    private class CycleConfigEntry extends ButtonConfigEntry {
        ...
        protected void updateConfigEntryButtonText() {
            ...
            if (hasBeenModified && (Onyxpannel.MODID + ".config.configGuiExplanations").equals(property.getLanguageKey())) {
    ...
    private class NumberSliderConfigEntry extends ButtonConfigEntry {
        ...
        private NumberSliderConfigEntry(Property property, OnyxConfigCategory.SubCategory.GuiSliderExtra guiSliderExtra) {
            super(property);
            ...
            updateConfigEntryButtonText();
            OnyxConfigGui.showDungeonPerformanceOverlayUntil = 0;
        }
