...
import de.onyxp.onyxpannel.command.OnyxCommand;
import de.onyxp.onyxpannel.command.TabCompletableCommand;
import de.onyxp.onyxpannel.config.gui.OnyxConfigGui;
import de.onyxp.onyxpannel.config.gui.OnyxConfigPreview;
import de.onyxp.onyxpannel.data.DataHelper;
import de.onyxp.onyxpannel.partyfinder.Rule;
...
public class OnyxConfig {
    ...
    public static String onyxCmdAlias;
    ...
    private static final List<OnyxConfigCategory> configCategories = new ArrayList<>();
    private final Onyxpannel main;
    private Property propOnyxCmdAlias;
    ...
    public OnyxConfig(Onyxpannel main, Configuration configuration) {
        this.main = main;
        cfg = configuration;
    ...
    private void updateConfigPreInit(String oldVersion) {
        if (oldVersion == null) {
            // config of Onyxpannel v1.8.9-0.10.2 and older

            // leave log search settings as is

            if (cfg.hasCategory(Configuration.CATEGORY_CLIENT)) {
                // copy old 'onyx' value to new, separate config
                if (cfg.hasKey(Configuration.CATEGORY_CLIENT, "onyx")) {
                    String oldOnyx = cfg.getString("onyx", Configuration.CATEGORY_CLIENT, "00000000-0000-0000-0000-000000000000", "Temporary config entry, should be deleted automatically.", Utils.VALID_UUID_PATTERN);
                    if (StringUtils.isNotEmpty(oldOnyx) && Utils.isValidUuid(oldOnyx)) {
                        // save into new cfg:
                        main.getOnyx().setOnyx(oldOnyx);
                    }
                }
    ...
    private void updateConfigPostInit(String oldVersion) {
        ...
        if ("1".equals(oldVersion)) {
            // config of Onyxpannel v1.8.9-0.12.0 and older
    ...
    private void syncConfig(boolean loadConfigFromFile, boolean readFieldsFromConfig, boolean saveToFile) {
        ...
        // Category: General
        OnyxConfigCategory configCat = new OnyxConfigCategory("General", "general");
        configCategories.add(configCat);

        // Sub-Category: Onyxpannel config gui
        OnyxConfigCategory.SubCategory subCat = configCat.addSubCategory("Onyxpannel config gui");
        ...
        subCat.addExplanations("Some features use the official Hypixel API and therefore require your API key.",
                "Use " + EnumChatFormatting.YELLOW + "/onyx apikey " + EnumChatFormatting.RESET + "to set your Hypixel API key",
                "The API key is stored " + EnumChatFormatting.ITALIC + "locally " + EnumChatFormatting.ITALIC + "on your computer.");
        subCat.addConfigEntry(main.getOnyx().getPropIsOnyxValid());

        // Sub-Category: Command settings
        subCat = configCat.addSubCategory("Command settings");

        propOnyxCmdAlias = subCat.addConfigEntry(cfg.get(configCat.getConfigName(),
                        "onyxCmdAlias", "m", "Alias for /onyx command")
        ...
        Property propWailaLevelOfDetail = subCat.addConfigEntry(cfg.get(configCat.getConfigName(),
                "wailaLevelOfDetail", "main info", "Level of detail of /onyx waila", new String[]{"main info", "all info"}));
        ...
        // Category: SkyBlock
        configCat = new OnyxConfigCategory("SkyBlock", "skyblock");
        configCategories.add(configCat);
        ...
        subCat.addExplanations("Servers usually restart once they exceed " + EnumChatFormatting.YELLOW + "30-38 ingame days " + EnumChatFormatting.RESET + "(10-13 hours)",
                "Use the command " + EnumChatFormatting.YELLOW + "/onyx worldage " + EnumChatFormatting.RESET + "to check how long the current world is loaded.",
        ...
        String analyzeCommand = "/onyx analyzeChests";
        ...
        OnyxConfigPreview nonStackableItemPreview = new OnyxConfigPreview(OnyxConfigPreview.createDemoItem("cake", "§dNew Year Cake", new String[]{"§7Given to every player as a", "§7celebration for the 1st SkyBlock", "§7year!", "", "§d§lSPECIAL"}, demoItemExtraAttributes));
        ...
        ItemStack demoAhItem = OnyxConfigPreview.createDemoItem("beacon", "§764x §fB§8e§facon Block", new String[]{"§f§lCOMMON", "§8§m-----------------", "§7Seller: §6[MVP§0++§6] Enlightener", "§7Buy it now: §63,900,000 coins", "", "§7Ends in: §e13h 33m 37s", "", "§eDon't click to inspect!"}, demoAhItemExtraAttributes);
        demoAhItem.stackSize = 64;
        OnyxConfigPreview ahItemPreview = new OnyxConfigPreview(demoAhItem);
        ...
        Property propBazaarSellAllOrder = subCat.addConfigEntry(cfg.get(configCat.getConfigName(),
                ...
                new OnyxConfigPreview(OnyxConfigPreview.createDemoItem("chest", "§aSell Inventory Now", new String[]{"§7Instantly sell anything in", "§7your inventory that can be", "§7sold on the Bazaar.", "", " §a1§7x §aEnchanted Leather §7for §65,263.1 coins", " §a42§7x §fLeather §7for §6436.8 coins", " §a2§7x §fRabbit Hide §7for §642.0 coins", " §a79§7x §fRaw Beef §7for §6450.3 coins", " §a16§7x §aEnchanted Raw Beef §7for §69,867.2 coins", "", "§7You earn: §615,698 coins", "", "§eClick to sell!"}, Collections.emptyMap())));
        ...
        OnyxConfigPreview bazaarGraphPreview = new OnyxConfigPreview(OnyxConfigPreview.createDemoItem("paper", "§aBuy Price §731d §77d §e24h", new String[]{
        ...
        OnyxConfigPreview bestiaryOverviewPreview = new OnyxConfigPreview(OnyxConfigPreview.createDemoItem("wheat", "§a§3The Barn", new String[]{
        ...
        // Category: SkyBlock Dungeons
        configCat = new OnyxConfigCategory("SkyBlock Dungeons", "skyblockdungeons");
        ...
        Property propDungOverlayPositionX = subCat.addConfigEntry(cfg.get(configCat.getConfigName(),
                ...
                null, "‰", // per mille
                (slider) -> {
                    OnyxConfig.dungOverlayPositionX = slider.getValueInt();
                    OnyxConfigGui.showDungeonPerformanceOverlayUntil = System.currentTimeMillis() + 500;
                });
        ...
        Property propDungOverlayPositionY = subCat.addConfigEntry(cfg.get(configCat.getConfigName(),
                ...
                null, "‰", // per mille
                (slider) -> {
                    OnyxConfig.dungOverlayPositionY = slider.getValueInt();
                    OnyxConfigGui.showDungeonPerformanceOverlayUntil = System.currentTimeMillis() + 500;
                });
        ...
        Property propDungOverlayGuiScale = subCat.addConfigEntry(cfg.get(configCat.getConfigName(),
                ...
                null, "%",
                (slider) -> {
                    OnyxConfig.dungOverlayGuiScale = slider.getValueInt();
                    OnyxConfigGui.showDungeonPerformanceOverlayUntil = System.currentTimeMillis() + 500;
                });
        ...
        Property propDungPartiesSize = subCat.addConfigEntry(cfg.get(configCat.getConfigName(),
                ...
                new OnyxConfigPreview(new OnyxChatComponent("Marked with: " + EnumChatFormatting.WHITE + "1 - 4").gray()));
        ...
        Property propDungDungeonReqMin = subCat.addConfigEntry(cfg.get(configCat.getConfigName(),
                ...
                new OnyxConfigPreview(new OnyxChatComponent("Marked with: " + EnumChatFormatting.DARK_RED + EnumChatFormatting.BOLD + "ᐯ").gray()));
        ...
        Property propDungClassMin = subCat.addConfigEntry(cfg.get(configCat.getConfigName(),
                        ...
                        .setMinValue(0).setMaxValue(50),
                new OnyxConfigPreview(new OnyxChatComponent("Marked with: " + EnumChatFormatting.RED + EnumChatFormatting.BOLD + "ᐯ").gray()));
        ...
        Property propDungMarkPartiesWithArcher = subCat.addConfigEntry(cfg.get(configCat.getConfigName(),
                        ...
                        new String[]{
                                ...
                                "do not mark"}),
                new OnyxConfigPreview(DataHelper.DungeonClass.ARCHER));
        ...
        Property propDungMarkPartiesWithBerserk = subCat.addConfigEntry(cfg.get(configCat.getConfigName(),
                        ...
                        new String[]{
                                ...
                                "if dupe: " + EnumChatFormatting.RED + "⬛", "always: " + EnumChatFormatting.RED + "⬛",
                                "do not mark"}),
                new OnyxConfigPreview(DataHelper.DungeonClass.BERSERK));
        ...
        Property propDungMarkPartiesWithHealer = subCat.addConfigEntry(cfg.get(configCat.getConfigName(),
                        ...
                        new String[]{
                                ...
                                "if dupe: " + EnumChatFormatting.RED + "⬛", "always: " + EnumChatFormatting.RED + "⬛",
                                "do not mark"}),
                new OnyxConfigPreview(DataHelper.DungeonClass.HEALER));
        ...
        Property propDungMarkPartiesWithMage = subCat.addConfigEntry(cfg.get(configCat.getConfigName(),
                        ...
                        new String[]{
                                ...
                                "if dupe: " + EnumChatFormatting.RED + "⬛", "always: " + EnumChatFormatting.RED + "⬛",
                                "do not mark"}),
                new OnyxConfigPreview(DataHelper.DungeonClass.MAGE));
        ...
        Property propDungMarkPartiesWithTank = subCat.addConfigEntry(cfg.get(configCat.getConfigName(),
                        ...
                        new String[]{
                                ...
                                "if dupe: " + EnumChatFormatting.RED + "⬛", "always: " + EnumChatFormatting.RED + "⬛",
                                "do not mark"}),
                new OnyxConfigPreview(DataHelper.DungeonClass.TANK));
        ...
        boolean modifiedOnyxCmdAlias = false;
        String onyxCmdAliasPreChange = onyxCmdAlias;
        ...
        if (readFieldsFromConfig) {
            ...
            onyxCmdAlias = propOnyxCmdAlias.getString();
            ...
            if (!StringUtils.equals(onyxCmdAliasPreChange, onyxCmdAlias)) {
                modifiedOnyxCmdAlias = true;
            }
        ...
        propOnyxCmdAlias.set(onyxCmdAlias);
        ...
        if (saveToFile && cfg.hasChanged()) {
            ...
            if (modifiedOnyxCmdAlias) {
                Map<String, ICommand> clientCommandsMap = ClientCommandHandler.instance.getCommands();
                ICommand possibleClientCommand = clientCommandsMap.get(onyxCmdAlias);
                if (possibleClientCommand != null && !(possibleClientCommand instanceof OnyxCommand)) {
                    ...
                    if (isPlayerIngame) {
                        main.getChatHelper().sendMessage(EnumChatFormatting.GOLD, " ⚠ " + EnumChatFormatting.GOLD + "Client-side commands from other mods cannot be used as a command alias. " + EnumChatFormatting.RED + "This would overwrite the other command! Therefore the alias for " + EnumChatFormatting.DARK_RED + "/onyx" + EnumChatFormatting.RED + " was not changed to " + EnumChatFormatting.DARK_RED + "/" + onyxCmdAlias);
                    }
                    onyxCmdAlias = onyxCmdAliasPreChange;
                    propOnyxCmdAlias.set(onyxCmdAlias);
                } else if (isPlayerIngame) {
                    if (StringUtils.isEmpty(onyxCmdAlias)) {
                        main.getChatHelper().sendMessage(EnumChatFormatting.RED, "Removed command alias for " + EnumChatFormatting.DARK_RED + "/onyx " + EnumChatFormatting.RED + "which takes effect after a game restart.");
                    } else {
                        main.getChatHelper().sendMessage(EnumChatFormatting.RED, "Changed command alias for " + EnumChatFormatting.DARK_RED + "/onyx " + EnumChatFormatting.RED + "to " + EnumChatFormatting.DARK_RED + "/" + onyxCmdAlias + EnumChatFormatting.RED + " which takes effect after a game restart.");
                    }
                }
            }
    ...
    public void theyOpenedTheConfigGui() {
        if (!hasOpenedConfigGui) {
            ...
            main.getChatHelper().sendMessage(new OnyxChatComponent(EnumChatFormatting.DARK_GREEN + " ❢ " + EnumChatFormatting.GREEN + "To configure " + EnumChatFormatting.GOLD + "Onyxpannel " + EnumChatFormatting.GREEN + "features use " + EnumChatFormatting.LIGHT_PURPLE + "/onyx config").green()
                    .setSuggestCommand("/onyx config")
                    .appendFreshSibling(new OnyxChatComponent("   (this message will be displayed only once)").gray()));
            hasOpenedConfigGui = true;
            syncFromFields();
        }
    }
    ...
    // other stuff
    public static List<OnyxConfigCategory> getConfigCategories() {
        return configCategories;
    }

    public Property getOnyxCmdAliasProperty() {
        return propOnyxCmdAlias;
    }
    ...
    public class ConfigEventHandler {
        ...
        public void onEvent(ConfigChangedEvent.OnConfigChangedEvent e) {
            if (Onyxpannel.MODID.equals(e.modID)) {
                syncFromGui();
            }
        }
    }
