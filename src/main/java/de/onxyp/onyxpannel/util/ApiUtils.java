...
import de.onyxp.onyxpannel.command.exception.ThrowingConsumer;
import de.onyxp.onyxpannel.config.CredentialStorage;
import de.onyxp.onyxpannel.data.Friend;
import de.onyxp.onyxpannel.data.HyPlayerData;
import de.onyxp.onyxpannel.data.HySkyBlockStats;
import de.onyxp.onyxpannel.data.HyStalkingData;
import de.onyxp.onyxpannel.error.ApiHttpErrorEvent;
import de.onyxp.onyxpannel.error.ApiHttpErrorException;
...
public class ApiUtils {
    ...
    private static BufferedReader makeApiCall(String url, boolean sendApiKey) throws IOException {
        return makeApiCall(url, sendApiKey, CredentialStorage.onyx);
    }

    private static BufferedReader makeApiCall(String url, boolean sendApiKey, String key) throws IOException {
        if (sendApiKey && !CredentialStorage.isOnyxValid) {
            throw new ApiHttpErrorException("Your current Hypixel API key is invalid. Use §4/onyx apikey §cto manually set your API key.", url, true);
        }
        try {
            ...
            if (connection.getResponseCode() == HttpStatus.SC_NO_CONTENT) { // http status 204
                ...
                throw new ApiHttpErrorException("Your current Hypixel API key seems to be invalid. Use §4/onyx apikey §cto manually set your API key.", url, true);
            ...
            if (cause instanceof ValidatorException && cause.getCause() instanceof SunCertPathBuilderException) {
                throw new ApiHttpErrorException("" + EnumChatFormatting.DARK_RED + EnumChatFormatting.BOLD + " ! "
                        + EnumChatFormatting.RED + "Java is outdated and doesn't support Let's Encrypt certificates (out of the box). A game restart might fix this issue. If the problem persists, open a ticket on the OnyxX discord server.", Onyxpannel.INVITE_URL);
            } else {
