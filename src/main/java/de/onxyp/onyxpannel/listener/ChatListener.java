...
import de.onyxp.onyxpannel.data.Friend;
...
public class ChatListener {
    ...
    public ChatListener(Onyxpannel main) {
        this.main = main;
    }
    ...
    public void onReplyToMsg(GuiScreenEvent.KeyboardInputEvent.Pre e) {
        ...
        if (OnyxConfig.fixReplyCmd && lastPMSender != null && e.gui instanceof GuiChat && lastTypedChars.length() < 3 && Keyboard.getEventKeyState()) {
    ...
    public void onChatMsgReceive(ClientChatReceivedEvent e) {
        ...
        if (privateMessageMatcher.find()) {
            ...
            if (CredentialStorage.isOnyxValid) {
                ...
                if (!joinedYourself && OnyxConfig.getDungPartyFinderPlayerLookupDisplay() != OnyxConfig.Setting.DISABLED) {
                ...
                } else if (joinedYourself && OnyxConfig.dungPartyFinderPartyLookup) {
                    // successfully joined another party via Dungeon Party Finder
                    main.getDungeonCache().lookupPartyMembers();
                }
            }
        } else if (CredentialStorage.isOnyxValid && OnyxConfig.dungPartyFullLookup && message.equals("Party Finder > Your dungeon group is full! Click here to warp to the dungeon!")
                && (Minecraft.getMinecraft().currentScreen == null || Minecraft.getMinecraft().currentScreen instanceof GuiChat)) {
            ClientCommandHandler.instance.executeCommand(Minecraft.getMinecraft().thePlayer, "/onyx dp");
        }
            ...
            if (stalkedPlayer != null && !stalkedPlayer.equals(Friend.FRIEND_NOT_FOUND)) {
                ApiUtils.fetchSkyBlockStats(stalkedPlayer, hySBStalking -> {
                    if (hySBStalking != null && hySBStalking.isSuccess()) {
                        ...
                        boolean outputAsChatMessages = OnyxConfig.getDungPartyFinderPlayerLookupDisplay() == OnyxConfig.Setting.TEXT;
                        ...
                        if (outputAsChatMessages) {
                            ...
                            // as a tooltip: == OnyxConfig.Setting.TOOLTIP
                            if (hasPlayedDungeons) {
                                // highest floor completions:
                        ...
                        ApiUtils.fetchHyPlayerDetails(stalkedPlayer, hyPlayerData -> {
                            ...
                            OnyxChatComponent armorLookupComponent;
                            if (outputAsChatMessages) {
                                armorLookupComponent = new OnyxChatComponent(skyBlockDetails + foundDungeonsSecrets).green();
                            } else {
                                armorLookupComponent = new OnyxChatComponent(armorLookupPrefix + EnumChatFormatting.GREEN + (playerName.endsWith("s") ? "'" : "'s") + " dungeons info (hover me)").green()
                                        .setHover(new OnyxChatComponent(skyBlockDetails + foundDungeonsSecrets));
                            }
