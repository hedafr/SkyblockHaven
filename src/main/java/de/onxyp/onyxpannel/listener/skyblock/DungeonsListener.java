...
import de.onyxp.onyxpannel.data.DataHelper.DungeonClass;
...
public class DungeonsListener {
    ...
    public DungeonsListener(Onyxpannel main) {
        this.main = main;
    ...
    public void onItemTooltip(ItemTooltipEvent e) {
        ...
        boolean toggleKeyBindingPressed = OnyxConfig.isDungeonItemTooltipToggleKeyBindingPressed() && !main.getDungeonCache().isInDungeon();
        OnyxConfig.Setting showItemQualityAndFloorDisplay = OnyxConfig.getShowItemQualityAndFloorDisplay();
        boolean showItemQualityAndFloor = showItemQualityAndFloorDisplay == OnyxConfig.Setting.ALWAYS
                || (showItemQualityAndFloorDisplay == OnyxConfig.Setting.SPECIAL && toggleKeyBindingPressed);
                ...
                if (lineMatcher.matches()) {
                    if (EnumChatFormatting.getTextWithoutFormattingCodes(lineMatcher.group("prefix")).equals("Gear Score: ")) {
                        ...
                        if (!hasCustomGearScore) {
                            if (OnyxConfig.dungItemQualityShortenNonRandomized) {
                        ...
                        if (showItemQualityAndFloor) {
                            if (OnyxConfig.isDungItemQualityAtTop()) {
                                if (OnyxConfig.dungItemHideGearScore) {
    ...
    private void renderPartyStatus(ItemStack item, int x, int y) {
        ...
        if (lastToolTipLine.endsWith("Complete previous floor first!")
            ...
            if (OnyxConfig.dungeonPartyMarker(activeDungeonClass).ifDuped()) {
            ...
            for (int i = 0; i < itemTooltip.size(); i++) {
                ...
                if (playerDetailMatcher.matches()) {
                    ...
                    if (OnyxConfig.dungeonPartyMarker(clazz) == OnyxConfig.Mark.BLOCK_ALWAYS) {
                    ...
                    if (classLevel < OnyxConfig.dungClassMin) {
                    ...
                    if (minDungLevelReq < OnyxConfig.dungDungeonReqMin) {
            ...
            if (partyType != DataHelper.PartyType.UNJOINABLE_OR_BLOCK) {
                ...
                for (Map.Entry<DungeonClass, AtomicInteger> partyClassInfo : dungClassesInParty.entrySet()) {
                    OnyxConfig.Mark dungeonPartyMarker = OnyxConfig.dungeonPartyMarker(partyClassInfo.getKey());
                    if (dungeonPartyMarker == OnyxConfig.Mark.UNIDEAL_DUPE && partyClassInfo.getValue().get() > 1) {
                        // 2+ class
                        dupedClasses.append(partyClassInfo.getKey().getShortName());
                    } else if (dungeonPartyMarker == OnyxConfig.Mark.UNIDEAL_ALWAYS) {
                        unwantedClasses.append(partyClassInfo.getKey().getShortName());
                    } else if (dungeonPartyMarker == OnyxConfig.Mark.BLOCK_DUPE && partyClassInfo.getValue().get() > 1) {
                ...
                if (partyType != DataHelper.PartyType.UNJOINABLE_OR_BLOCK) {
                    ...
                    if (OnyxConfig.dungPartiesSize && partySize > 0) {
                        GlStateManager.pushMatrix();
                        GlStateManager.translate(0, 0, 280);
        ...
        if (OnyxConfig.dungPartyFinderOverlayDrawBackground &&
                (partyType != DataHelper.PartyType.CURRENT
    ...
    public void onGuiInit(GuiScreenEvent.InitGuiEvent.Post e) {
        if (OnyxConfig.dungPartyFinderRuleEditorShowOpenButton && e.gui instanceof GuiChest) {
    ...
    public void onGuiButtonClick(GuiScreenEvent.ActionPerformedEvent.Post e) {
        if (OnyxConfig.dungPartyFinderRuleEditorShowOpenButton && e.gui instanceof GuiChest && e.button.id == 8765 && "Rule editor ↗".equals(e.button.displayString)) {
    ...
    public void onGuiButtonHover(GuiScreenEvent.DrawScreenEvent.Post e) {
        if (OnyxConfig.dungPartyFinderRuleEditorShowOpenButton && e.gui instanceof GuiChest && btnOpenRuleEditor != null && btnOpenRuleEditor.isMouseOver()) {
            Minecraft mc = Minecraft.getMinecraft();
            List<String> btnTooltip = new ArrayList<>();
            btnTooltip.add("Open Onyxpannel's Party Notes Rule Editor");
            btnTooltip.add(EnumChatFormatting.GRAY + "Want to remove this button? " + EnumChatFormatting.YELLOW + "/onyx config editor");
            btnTooltip.add(EnumChatFormatting.GRAY + "Want to remove the overlay inside dungeons? " + EnumChatFormatting.YELLOW + "/onyx config overlay");
    ...
    public void onMessageReceived(ClientChatReceivedEvent e) {
        if (e.type != 2) { // normal chat or system msg (not above action bar)
            ...
            if (!main.getDungeonCache().isInDungeon()) {
                ...
                } else if (OnyxConfig.dungSendWrongFloorWarning) {
                    ...
                    if (dungeonEnteredMatcher.matches()) {
                        ...
                        if (queuedFloor != null && !queuedFloor.equals(floor)) {
                            ...
                            new TickDelay(() -> {
                                ...
                                Minecraft.getMinecraft().thePlayer.playSound("mob.onyx.hurt", Minecraft.getMinecraft().gameSettings.getSoundLevel(SoundCategory.MASTER), 1);
                            }, 100);
                        }
                    }
                }
            ...
            if (dungeonDeathMatcher.matches()) {
                ...
                // dungeon "end screen"
                if (OnyxConfig.dungSendPerformanceOnEndScreen) {
        ...
        if (e.gui instanceof GuiChest) {
            ...
            if (inventory.getName().equals("Party Finder")) {
                ...
                if (hoveredSlot != null && hoveredSlot.getHasStack()) {
                    ...
                    if (CredentialStorage.isOnyxValid && OnyxConfig.dungPartyFinderPartyLookup) {
    ...
    public void onRenderGameOverlay(RenderGameOverlayEvent.Post e) {
        if (e.type == RenderGameOverlayEvent.ElementType.ALL) {
            ...
            boolean isEditingDungeonOverlaySettings = OnyxConfigGui.showDungeonPerformanceOverlay();
            if (OnyxConfig.dungOverlayEnabled && dungeonCache.isInDungeon() || isEditingDungeonOverlaySettings) {
                ...
                if (!OnyxConfig.hasOpenedConfigGui) {
                    dungeonPerformanceEntries.add(EnumChatFormatting.RED + " ⚠ Want to move me? " + EnumChatFormatting.DARK_GRAY + "➡ " + EnumChatFormatting.LIGHT_PURPLE + "/onyx config overlay");
                }

                FontRenderer fontRenderer = Minecraft.getMinecraft().fontRendererObj;
                GlStateManager.pushMatrix();

                float scaleFactor = OnyxConfig.dungOverlayGuiScale / 100f;
                GlStateManager.scale(scaleFactor, scaleFactor, 0);
                for (int line = 0; line < dungeonPerformanceEntries.size(); line++) {
                    String dungeonPerformanceEntry = dungeonPerformanceEntries.get(line);
                    int xPos = (int) ((e.resolution.getScaledWidth() * (OnyxConfig.dungOverlayPositionX / 1000d)) / scaleFactor);
                    int yPos = (int) ((e.resolution.getScaledHeight() * (OnyxConfig.dungOverlayPositionY / 1000d) + 2) / scaleFactor + fontRenderer.FONT_HEIGHT * line + 2);
                    OnyxConfig.Setting dungOverlayTextBorder = OnyxConfig.getDungOverlayTextBorder();
                    if (dungOverlayTextBorder == OnyxConfig.Setting.TEXT) {
                        // normal mc text shadow (drop shadow)
                        fontRenderer.drawStringWithShadow(dungeonPerformanceEntry, xPos, yPos, 0xffFFAA00);
                    } else if (dungOverlayTextBorder == OnyxConfig.Setting.ALWAYS) {
                        // full outline
