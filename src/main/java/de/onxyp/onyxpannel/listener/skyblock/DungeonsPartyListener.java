...
public class DungeonsPartyListener {
    ...
    public DungeonsPartyListener(Onyxpannel main) {
    ...
    public DungeonsPartyListener(Onyxpannel main, List<String> partyMembers) {
        if (nextStep == Step.STOP) { // prevent double-registration of this listener
            this.main = main;
    ...
    private void sendDungeonPartyStats() {
        ...
        OnyxChatComponent dungeonsParty = new OnyxChatComponent("Dungeons party").gold().bold();
        ...
        for (Map.Entry<String, Optional<UUID>> partyMember : partyMembers.entrySet()) {
            if (playerEntry.length() > 0) {
                ...
                OnyxChatComponent dungeonPartyEntry = new OnyxChatComponent(playerEntry.toString())
                        .setSuggestCommand((partyMemberName.equals(thePlayerName) ? "/boop " : "/p kick ") + partyMemberName, false);
                if (playerTooltip.length() > 0) {
                    dungeonPartyEntry.setHover(new OnyxChatComponent(playerTooltip.toString()));
                }
            ...
            if (sbStats != null && sbStats.isSuccess()) {
                ...
                if (selectedClass != null) {
                    ...
                    classAndDungeonTypeInfo = selectedClass.getName() + " " + (OnyxConfig.useRomanNumerals() ? Utils.convertArabicToRoman(selectedClassLevel) : selectedClassLevel);
                }
