package de.onyxp.onyxpannel.listener.skyblock;
...
import de.onyxp.onyxpannel.data.BestiaryEntry;
import de.onyxp.onyxpannel.data.DataHelper;
import de.onyxp.onyxpannel.data.HySkyBlockStats;
import de.onyxp.onyxpannel.data.XpTables;
import de.onyxp.onyxpannel.util.GsonUtils;
...
public class SkyBlockListener {
    ...
    public SkyBlockListener(Onyxpannel main) {
        this.main = main;
    ...
    public void onItemLookupInGui(GuiScreenEvent.KeyboardInputEvent.Pre e) {
        ...
        if (OnyxConfig.isLookupWikiKeyBindingPressed()) {
            itemLookupType = ItemLookupType.WIKI;
        } else if (OnyxConfig.isLookupOfficialWikiKeyBindingPressed()) {
            itemLookupType = ItemLookupType.OFFICIAL_WIKI;
        } else if (OnyxConfig.isLookupPriceKeyBindingPressed()) {
        ...
        if (itemLookupType != ItemLookupType.INVALID && Keyboard.getEventKeyState() && e.gui instanceof GuiContainer) {
            ...
            if (hoveredSlot != null && hoveredSlot.getHasStack()) {
                ...
                if (itemBaseName != null) {
                    ...
                    main.getChatHelper().sendMessage(new OnyxChatComponent(EnumChatFormatting.DARK_GREEN + " ➡ "
                            + EnumChatFormatting.GREEN + "Open" + (OnyxConfig.lookupItemDirectly ? "ing " : " ") + itemLookupType.getDescription() + " for " + itemBaseName).green()
                            .setUrl(link, itemLookupType.description + ": " + EnumChatFormatting.WHITE + link));

                    if (OnyxConfig.lookupItemDirectly) {
    ...
    public void onItemTooltip(ItemTooltipEvent e) {
        ...
        if ((OnyxConfig.getBazaarConnectGraphsNodes() == OnyxConfig.Setting.ALWAYS
                || OnyxConfig.getBazaarConnectGraphsNodes() == OnyxConfig.Setting.SPECIAL && OnyxConfig.isTooltipToggleKeyBindingPressed())
            ...
            } else if (currentScreen instanceof OnyxConfigGui) {
        ...
        if ((OnyxConfig.getTooltipPetExpDisplay() == OnyxConfig.Setting.ALWAYS
                || OnyxConfig.getTooltipPetExpDisplay() == OnyxConfig.Setting.SPECIAL && OnyxConfig.isTooltipToggleKeyBindingPressed())
        ...
        OnyxConfig.Setting tooltipItemAgeDisplay = OnyxConfig.getTooltipItemAgeDisplay();
        OnyxConfig.Setting tooltipItemTimestampDisplay = OnyxConfig.getTooltipItemTimestampDisplay();

        // add item age to tooltip
        if (extraAttributes != null && extraAttributes.hasKey("timestamp")
                && (tooltipItemAgeDisplay != OnyxConfig.Setting.DISABLED || tooltipItemTimestampDisplay != OnyxConfig.Setting.DISABLED)) {
            ...
            if (localZoneDateTime != null) {
                ...
                switch (tooltipItemTimestampDisplay) {
                    case SPECIAL:
                        if (!OnyxConfig.isTooltipToggleKeyBindingPressed()) {
                            break;
                        }
                    case ALWAYS:
                ...
                switch (tooltipItemAgeDisplay) {
                    ...
                    case ALWAYS: {
                        ...
                        String itemAge = itemAgeInMs >= 60_000
                                ? (OnyxConfig.tooltipItemAgeShortened ? Utils.getDurationAsWord(itemCreationTimestamp) : Utils.getDurationAsWords(itemCreationTimestamp).first())
                                : "<1 minute";
        ...
        // bestiary overview:
        String bestiaryOverviewOrder = OnyxConfig.bestiaryOverviewOrder;
        ...
        if (currentScreen instanceof GuiChest && isBestiaryOverviewVisible && isBestiaryMainGui((GuiChest) currentScreen, e.itemStack)) {
            e.toolTip.add("");
            e.toolTip.add("" + EnumChatFormatting.GOLD + EnumChatFormatting.BOLD + EnumChatFormatting.UNDERLINE + "Onyxpannel: Bestiary overview");
            ...
            e.toolTip.add(EnumChatFormatting.DARK_GRAY + " & Onyxpannel tooltip shows kills " + EnumChatFormatting.ITALIC + "left" + EnumChatFormatting.RESET + EnumChatFormatting.DARK_GRAY + "!)");
        ...
        } else if (currentScreen instanceof OnyxConfigGui && isBestiaryOverviewVisible && e.itemStack.getItem() == Items.wheat) {
            if (isDifferentTriggerItem) {
                ...
                bestiaryOverview.add(new BestiaryEntry(EnumChatFormatting.GREEN + "Onyx", true));
            }
        } else {
        ...
        if ((e.itemStack.getItem() == Items.cauldron || e.itemStack.getItem() == Item.getItemFromBlock(Blocks.chest)) && !"unordered".equals(OnyxConfig.bazaarSellAllOrder)) {
            ...
            if ("Sell Inventory Now".equals(hoveredItemName) || "Sell Sacks Now".equals(hoveredItemName)) {
                ...
                if ("high → low".equals(OnyxConfig.bazaarSellAllOrderAscDesc)) {
                ...
                for (int i = 0; i < toolTip.size(); i++) {
                    ...
                    if (bazaarSellMatcher.matches()) {
                        ...
                        try {
                            double key;
                            String suffix = "";
                            switch (OnyxConfig.bazaarSellAllOrder) {
                                case "price (sum)":
                ...
                if (sellEntries.size() > 1) {
                    ...
                    e.toolTip.add(startIndex + sellEntryIndex, EnumChatFormatting.DARK_GRAY + "  » ordered by " + OnyxConfig.bazaarSellAllOrder);
                }
            }
        }

        // bazaar: show how many items left on offer/order
        if (OnyxConfig.bazaarShowItemsLeft) {
        ...
        OnyxConfig.Setting tooltipAuctionHousePriceEachDisplay = OnyxConfig.getTooltipAuctionHousePriceEachDisplay();
        if ((tooltipAuctionHousePriceEachDisplay == OnyxConfig.Setting.ALWAYS || tooltipAuctionHousePriceEachDisplay == OnyxConfig.Setting.SPECIAL && OnyxConfig.isTooltipToggleKeyBindingPressed())
                && (e.entityPlayer.openContainer instanceof ContainerChest || currentScreen instanceof OnyxConfigGui)) {
                ...
                if (toolTipLineUnformatted.startsWith("Top bid: ")
                    ...
                    try {
                        ...
                        if (enchantmentName != null) {
                            toolTip.add(i + 1, EnumChatFormatting.YELLOW + "  (≙ " + itemAmount + "x " + EnumChatFormatting.BLUE + enchantmentName + " " + (OnyxConfig.useRomanNumerals() ? "I" : "1") + EnumChatFormatting.RESET + EnumChatFormatting.YELLOW + " for " + formattedPriceEach + " each)");
                        } else {
    ...
    public void onRenderGuiBackground(GuiScreenEvent.DrawScreenEvent.Pre e) {
        if (e.gui instanceof GuiChest) {
            OnyxConfig.Setting markAuctionHouseEndedAuctions = OnyxConfig.getMarkAuctionHouseEndedAuctions();
            if (markAuctionHouseEndedAuctions == OnyxConfig.Setting.DISABLED) {
                return;
            }
                        ...
                        if (itemNbtDisplay != null && itemNbtDisplay.hasKey("Lore", Constants.NBT.TAG_LIST)) {
                            ...
                            for (int loreLineNr = loreList.tagCount() - 1; loreLineNr >= 0; --loreLineNr) {
                                ...
                                if (loreLine.startsWith("Ends in: ")) {
                                    ...
                                    auctionStatus = EnumChatFormatting.RED + (markAuctionHouseEndedAuctions == OnyxConfig.Setting.TEXT ? "Expired" : "E");
                                } else if (loreLine.equals("Status: Ended!")) {
                                    auctionStatus = EnumChatFormatting.GREEN + (markAuctionHouseEndedAuctions == OnyxConfig.Setting.TEXT ? "Ended" : "E");
                                } else if (loreLine.equals("Status: Sold!")) {
                                    auctionStatus = EnumChatFormatting.GREEN + (markAuctionHouseEndedAuctions == OnyxConfig.Setting.TEXT ? "Sold" : "S");
                                } else {
                                    continue;
                                }
                                GlStateManager.pushMatrix();
                                GlStateManager.translate(0, 0, 281);
                                double scaleFactor = markAuctionHouseEndedAuctions == OnyxConfig.Setting.TEXT ? 0.4 : 0.5;
    ...
    private Pair<String, Integer> extractEnchantedBookInfo(GuiScreen currentScreen, ItemStack currentItem) {
        ...
        if (!shouldDisplayPriceEach && inventoryName.startsWith("Enchantments ➜ ")) {
            if (currentItemName.contains("Turbo-")) {
                ...
                for (String priceEachEnchantment : OnyxConfig.tooltipAuctionHousePriceEachEnchantments) {
