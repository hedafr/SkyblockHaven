package de.onyxp.onyxpannel.chesttracker;

import de.onyxp.onyxpannel.Onyxpannel;
import de.onyxp.onyxpannel.chesttracker.data.HyBazaarData;
import de.onyxp.onyxpannel.chesttracker.data.HyItemsData;
import de.onyxp.onyxpannel.chesttracker.data.ItemData;
import de.onyxp.onyxpannel.chesttracker.data.LowestBinsCache;
import de.onyxp.onyxpannel.data.DataHelper;
import de.onyxp.onyxpannel.data.HySkyBlockStats;
import de.onyxp.onyxpannel.util.ApiUtils;
...
public class ChestTracker {
    ...
    public ChestTracker(Onyxpannel main) {
        this.main = main;
        refreshPriceCache();
        ...
        if (allowUpdateBazaar()) {
            ...
            ApiUtils.fetchBazaarData(bazaarData -> {
                if (bazaarData == null || !bazaarData.isSuccess()) {
                    main.getChatHelper().sendMessage(new OnyxChatComponent("Error: Couldn't get Bazaar data from Hypixel API! API might be down: check status.hypixel.net").red().setUrl("https://status.hypixel.net/"));
                }
        ...
        if (allowUpdateLowestBins()) {
            ...
            ApiUtils.fetchLowestBins(lowestBins -> {
                if (!lowestBins.hasData()) {
                    main.getChatHelper().sendMessage(new OnyxChatComponent("Error: Couldn't get lowest BINs from Moulberry's API! API might be down: check if " + ApiUtils.LOWEST_BINS + " is reachable.").red().setUrl(ApiUtils.LOWEST_BINS));
                }
        ...
        if (allowUpdateNpcSell()) {
            ...
            ApiUtils.fetchItemsData(itemsData -> {
                ...
                if (itemsData == null || !itemsData.isSuccess()) {
                    main.getChatHelper().sendMessage(new OnyxChatComponent("Error: Couldn't get Items data from Hypixel API! API might be down: check status.hypixel.net").red().setUrl("https://status.hypixel.net/"));
                } else {
    ...
    public void markChestsWithWantedItem(String sbKey, int amount, String itemName) {
        ...
        final OnyxChatComponent relevantChestCoordsHover = new OnyxChatComponent("Chests with ").gold().bold().appendSibling(new OnyxChatComponent(itemName).reset())
                .appendFreshSibling(new OnyxChatComponent(StringUtils.repeat(' ', maxItemCountLength) + "        (x | y | z)").gray());
        ...
        chestsWithWantedItemsAndCount.entrySet().stream()
                ...
                .forEach(wantedChest -> {
                    ...
                    relevantChestCoordsHover.appendFreshSibling(new OnyxChatComponent(" " + EnumChatFormatting.YELLOW + itemCountInChest + EnumChatFormatting.DARK_GRAY + "x ➜ "
        ...
        if (relevantChestCount > relevantChestsCoordsLimit) {
            relevantChestCoordsHover.appendFreshSibling(new OnyxChatComponent("      + " + (relevantChestCount - relevantChestsCoordsLimit) + " more chests").gray());
        }
        main.getChatHelper().sendMessage(new OnyxChatComponent("Chest Tracker & Analyzer is now highlighting " + EnumChatFormatting.LIGHT_PURPLE + relevantChestCount + EnumChatFormatting.GREEN + " chest" + (relevantChestCount > 1 ? "s" : "") + " with " + itemName
                + EnumChatFormatting.DARK_GREEN + " [hover for coords, click to re-open GUI]").green()
                .setHover(relevantChestCoordsHover)
                .setSuggestCommand("/onyx analyzeChests", false));
