...
import de.onyxp.onyxpannel.chesttracker.ChestOverviewGui;
import de.onyxp.onyxpannel.command.exception.ApiContactException;
import de.onyxp.onyxpannel.command.exception.InvalidPlayerNameException;
import de.onyxp.onyxpannel.command.exception.OnyxCommandException;
import de.onyxp.onyxpannel.config.CredentialStorage;
import de.onyxp.onyxpannel.config.OnyxConfig;
import de.onyxp.onyxpannel.config.gui.OnyxConfigBestFriendsMigration;
import de.onyxp.onyxpannel.config.gui.OnyxConfigGui;
import de.onyxp.onyxpannel.data.*;
import de.onyxp.onyxpannel.data.HySkyBlockStats.Profile.Pet;
import de.onyxp.onyxpannel.handler.DungeonCache;
import de.onyxp.onyxpannel.listener.skyblock.DungeonsPartyListener;
import de.onyxp.onyxpannel.partyfinder.RuleEditorGui;
import de.onyxp.onyxpannel.search.GuiSearch;
...
public class OnyxCommand extends CommandBase {
    ...
    public OnyxCommand(Onyxpannel main) {
        this.main = main;
    }
    ...
    public List<String> getCommandAliases() {
        ...
        if (StringUtils.isNotEmpty(OnyxConfig.onyxCmdAlias)) {
            aliases.add(OnyxConfig.onyxCmdAlias);
        }
        return aliases;
    }
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        ...
        //region sub commands: Best friends, friends & other players
            ...
            // work-around so you can still say 'onyx' in chat without triggering the client-side command
            ...
            displayGuiScreen(new OnyxConfigBestFriendsMigration());
            ...
            OnyxChatComponent confirmationMsg = new OnyxChatComponent("[§2Onyxpannel§a] The 'login & logout notification filter' removal info message will no longer be shown.").green();
            if (main.getFriendsHandler().getBestFriendsListSize() > 0) {
                confirmationMsg.appendSibling(new OnyxChatComponent("\nHowever, don't forget to migrate your Onyxpannel best friends by using §4/onyx bestfriends§c!").red())
                        .setSuggestCommand("/onyx bestfriends");
            }
        ...
        //region sub commands: SkyBlock
            ...
            boolean showAllInfo = OnyxConfig.keepFullWailaInfo();
            if (args.length == 2) {
        ...
        //region sub-commands: miscellaneous
            ...
            displayGuiScreen(new OnyxConfigGui(buildString(args, 1)));
            ...
            main.getChatHelper().sendMessage(new OnyxChatComponent("➜ Need help with " + EnumChatFormatting.GOLD + Onyxpannel.MODNAME + EnumChatFormatting.GREEN + "? Do you have any questions, suggestions or other feedback? " + EnumChatFormatting.GOLD + "Join the OnyxX discord!").green().setUrl(Onyxpannel.INVITE_URL));
        }
        //endregion
        //region sub-commands: update mod
            ...
            if (directory == null) {
                main.getChatHelper().sendMessage(new OnyxChatComponent("[§2Onyxpannel§a] open directory:").green()
                        .appendFreshSibling(new OnyxChatComponent(" §6➊ §a/config/onyxpannel/").setSuggestCommand("/" + getCommandName() + " directory config"))
                        .appendFreshSibling(new OnyxChatComponent(" §6➋ §a/mods/").setSuggestCommand("/" + getCommandName() + " directory mods")));
                return;
            }
            try {
                ...
                e.printStackTrace();
                throw new OnyxCommandException("✖ An error occurred trying to open the mod's directory. I guess you have to open it manually ¯\\_(ツ)_/¯");
            }
        }
        //endregion
        // help
        ...
        // fix: run server-side command /m with optional arguments
            ...
            Minecraft.getMinecraft().thePlayer.sendChatMessage("/" + OnyxConfig.onyxCmdAlias + cmdArgs);
        }
        // "catch-all" remaining sub-commands
            ...
            main.getChatHelper().sendMessage(EnumChatFormatting.RED, "Command " + EnumChatFormatting.DARK_RED + "/" + getCommandName() + " " + args[0] + EnumChatFormatting.RED + " doesn't exist. Use " + EnumChatFormatting.DARK_RED + "/" + getCommandName() + " help " + EnumChatFormatting.RED + "to show command usage."
                    + (StringUtils.isNotEmpty(OnyxConfig.onyxCmdAlias) ? "\n" + EnumChatFormatting.RED + "Are you trying to use a server-side command " + EnumChatFormatting.DARK_RED + "/" + OnyxConfig.onyxCmdAlias + EnumChatFormatting.RED + "? Use " + EnumChatFormatting.DARK_RED + "/" + OnyxConfig.onyxCmdAlias + " cmd [arguments] " + EnumChatFormatting.RED + "instead." : ""));
        }
    }

    //region sub commands: Best friends, friends & other players
    private void handleStalking(String[] args) throws CommandException {
        if (!CredentialStorage.isOnyxValid) {
            throw new OnyxCommandException("[Onyxpannel] You haven't set your Hypixel API key yet or the API key is invalid. Use " + EnumChatFormatting.DARK_RED + "/" + this.getCommandName() + " apikey " + EnumChatFormatting.RED + "to manually set your existing API key.");
        }
        if (args.length != 2) {
    ...
    private void handleBestFriendRemove(String[] args) throws CommandException {
        ...
        if (removed) {
            ...
            throw new OnyxCommandException(EnumChatFormatting.DARK_RED + username + EnumChatFormatting.RED + " isn't a best friend.");
        }
    }
    //endregion
        ...
        ApiUtils.fetchSkyBlockStats(stalkedPlayer, hySBStalking -> {
            if (hySBStalking != null && hySBStalking.isSuccess()) {
                ...
                if (activeProfile == null) {
                    throw new OnyxCommandException("Looks like " + EnumChatFormatting.DARK_RED + stalkedPlayer.getName() + EnumChatFormatting.RED + " hasn't played SkyBlock yet.");
                }

                String highestSkill = null;
                int highestLevel = -1;

                OnyxChatComponent skillLevels = new OnyxChatComponent("Skill levels:").gold();
                ...
                for (Map.Entry<XpTables.Skill, Integer> entry : member.getSkills().entrySet()) {
                    ...
                    String skillLevel = OnyxConfig.useRomanNumerals() ? Utils.convertArabicToRoman(level) : String.valueOf(level);
                    skillLevels.appendFreshSibling(new OnyxChatComponent.KeyValueTooltipComponent(skill, skillLevel));
                ...
                OnyxChatComponent wealthHover = new OnyxChatComponent("Accessible coins:").gold()
                        .appendFreshSibling(new OnyxChatComponent.KeyValueTooltipComponent("Purse", Utils.formatNumberWithAbbreviations(member.getCoinPurse())))
                        .appendFreshSibling(new OnyxChatComponent.KeyValueTooltipComponent("Bank", (activeProfile.getCoinBank() != -1) ? Utils.formatNumberWithAbbreviations(activeProfile.getCoinBank()) : "API access disabled"));
                if (activeProfile.coopCount() > 0) {
                    wealthHover.appendFreshSibling(new ChatComponentText(" "));
                    wealthHover.appendFreshSibling(new OnyxChatComponent.KeyValueTooltipComponent("Co-op members", String.valueOf(activeProfile.coopCount())));
                    wealthHover.appendFreshSibling(new OnyxChatComponent.KeyValueTooltipComponent("Co-ops' purses sum", Utils.formatNumberWithAbbreviations(activeProfile.getCoopCoinPurses(stalkedPlayer.getUuid()))));
                }

                String gameModeIcon = activeProfile.getGameModeIcon();
                OnyxChatComponent sbStats = new OnyxChatComponent("SkyBlock stats of " + stalkedPlayer.getName() + EnumChatFormatting.RESET + EnumChatFormatting.GRAY + " (" + (gameModeIcon.isEmpty() ? "" : gameModeIcon + EnumChatFormatting.GRAY + ", ") + activeProfile.getCuteName() + ")").gold().bold().setUrl("https://sky.shiiyu.moe/stats/" + stalkedPlayer.getName() + "/" + activeProfile.getCuteName(), "Click to view SkyBlock stats on sky.shiiyu.moe")
                        .appendFreshSibling(new OnyxChatComponent.KeyValueChatComponent("Coins", coinsBankAndPurse).setHover(wealthHover));
                // highest skill + skill average:
                if (highestSkill != null) {
                    if (highestLevel == 0) {
                        sbStats.appendFreshSibling(new OnyxChatComponent.KeyValueChatComponent("Highest Skill", "All skills level 0"));
                    } else {
                        String highestSkillLevel = OnyxConfig.useRomanNumerals() ? Utils.convertArabicToRoman(highestLevel) : String.valueOf(highestLevel);
                        sbStats.appendFreshSibling(new OnyxChatComponent.KeyValueChatComponent("Highest Skill", highestSkill + " " + highestSkillLevel).setHover(skillLevels));
                    }
                    double skillAverage = XpTables.Skill.getSkillAverage(skillLevelsSum);
                    sbStats.appendFreshSibling(new OnyxChatComponent.KeyValueChatComponent("Skill average", String.format("%.1f", skillAverage))
                            .setHover(new OnyxChatComponent("Average skill level over all non-cosmetic skills\n(all except Runecrafting and Social)").gray()));
                } else {
                    sbStats.appendFreshSibling(new OnyxChatComponent.KeyValueChatComponent("Highest Skill", "API access disabled"));
                }

                // slayer levels:
                StringBuilder slayerLevels = new StringBuilder();
                StringBuilder slayerLevelsTooltip = new StringBuilder();
                OnyxChatComponent slayerLevelsTooltipComponent = new OnyxChatComponent("Slayer bosses:").gold();
                for (Map.Entry<XpTables.Slayer, Integer> entry : member.getSlayerLevels().entrySet()) {
                    ...
                    String slayerLevel = (level > 0) ? (OnyxConfig.useRomanNumerals() ? Utils.convertArabicToRoman(level) : String.valueOf(level)) : "0";
                    slayerLevels.append(slayerLevel);
                }
                OnyxChatComponent slayerLevelsComponent = new OnyxChatComponent.KeyValueChatComponent("Slayer levels", slayerLevels.toString());
                slayerLevelsComponent.setHover(slayerLevelsTooltipComponent.appendFreshSibling(new OnyxChatComponent(slayerLevelsTooltip.toString()).white()));
                sbStats.appendFreshSibling(slayerLevelsComponent);

                // dungeons:
                OnyxChatComponent dungeonsComponent = null;
                ...
                if (hasPlayedDungeons) {
                    OnyxChatComponent dungeonHover = new OnyxChatComponent("Dungeoneering").gold().bold();
                    ...
                    if (selectedClass != null) {
                        ...
                        selectedDungClass = selectedClass.getName() + " " + (OnyxConfig.useRomanNumerals() ? Utils.convertArabicToRoman(selectedClassLevel) : selectedClassLevel);
                    }
                    dungeonsComponent = new OnyxChatComponent.KeyValueChatComponent("Dungeoneering", selectedDungClass)
                    ...
                    if (classLevels != null && !classLevels.isEmpty()) {
                        dungeonHover.appendFreshSibling(new OnyxChatComponent("Classes:").gold());
                        for (Map.Entry<DataHelper.DungeonClass, Integer> classEntry : classLevels.entrySet()) {
                            String classLevel = (OnyxConfig.useRomanNumerals() ? Utils.convertArabicToRoman(classEntry.getValue()) : String.valueOf(classEntry.getValue()));
                            dungeonHover.appendFreshSibling(new OnyxChatComponent.KeyValueChatComponent((classEntry.getKey() == selectedClass ? "➜ " : "   ") + classEntry.getKey().getName(), classLevel));
                        }
                    }
                    ...
                    if (dungeonTypes != null && !dungeonTypes.isEmpty()) {
                        ...
                        for (Map.Entry<String, HySkyBlockStats.Profile.Dungeons.Type> dungeonTypeEntry : dungeonTypes.entrySet()) {
                            ...
                            dungeonsComponent.appendFreshSibling(new OnyxChatComponent.KeyValueChatComponent("   " + dungeonTypeName, dungeonType.getSummary(isMasterFloor)))
                                    .setHover(dungeonHover);
                            // dungeon type entry for tooltip
                            if (isMasterFloor) {
                                dungeonHover.appendFreshSibling(new OnyxChatComponent(dungeonTypeName).gold());
                            } else {
                                // non-master dungeon
                                int dungeonTypeLevel = dungeonTypeEntry.getValue().getLevel();
                                dungeonHover.appendFreshSibling(new OnyxChatComponent.KeyValueChatComponent(dungeonTypeName, "Level " + (OnyxConfig.useRomanNumerals() ? Utils.convertArabicToRoman(dungeonTypeLevel) : dungeonTypeLevel)));
                            }
                            ...
                            for (Map.Entry<String, StringBuilder> floorInfo : floorStats.entrySet()) {
                                dungeonHover.appendFreshSibling(new OnyxChatComponent.KeyValueChatComponent("   Floor " + floorInfo.getKey(), floorInfo.getValue().toString()));
                            }
                        }
                        dungeonHover.appendFreshSibling(new OnyxChatComponent(" Floor nr: completed / total floors (best score)").gray().italic());
                    }
                }
                if (!hasPlayedDungeons) {
                    dungeonsComponent = new OnyxChatComponent.KeyValueChatComponent("Dungeons", EnumChatFormatting.ITALIC + "never played");
                }
                ...
                OnyxChatComponent petsComponent = null;
                if (activePet != null) {
                    petsComponent = new OnyxChatComponent.KeyValueChatComponent("Active Pet", activePet.toFancyString());
                } else if (bestPet != null) {
                    petsComponent = new OnyxChatComponent.KeyValueChatComponent("Best Pet", bestPet.toFancyString());
                }
                if (pets.length() > 0 && petsComponent != null) {
                    petsComponent.setHover(new OnyxChatComponent("Other pets:").gold().bold().appendFreshSibling(new OnyxChatComponent(pets.toString())));
                }
                if (petsComponent == null) {
                    petsComponent = new OnyxChatComponent.KeyValueChatComponent("Pet", "none");
                }
                ...
                OnyxChatComponent.KeyValueChatComponent uniqueMinionsComponent = new OnyxChatComponent.KeyValueChatComponent("Unique Minions", uniqueMinions);
                if (uniqueMinionsHoverText != null) {
                    uniqueMinionsComponent.setHover(new OnyxChatComponent(uniqueMinionsHoverText).gray());
                }
                sbStats.appendFreshSibling(uniqueMinionsComponent);
                // fairy souls:
                sbStats.appendFreshSibling(new OnyxChatComponent.KeyValueChatComponent("Fairy Souls", (member.getFairySoulsCollected() >= 0) ? String.valueOf(member.getFairySoulsCollected()) : "API access disabled"));
                // profile age:
                sbStats.appendFreshSibling(new OnyxChatComponent.KeyValueChatComponent("Profile age", fancyFirstJoined.first()).setHover(new OnyxChatComponent.KeyValueTooltipComponent("Join date", (fancyFirstJoined.second() == null ? "today" : fancyFirstJoined.second()))));
    ...
    private void handleAnalyzeChests(String[] args) {
        if (args.length == 1) {
            ...
            if (enabledChestTracker) {
                ...
                if (OnyxConfig.chestAnalyzerShowCommandUsage) {
                    main.getChatHelper().sendMessage(new OnyxChatComponent("Enabled chest tracker! You can now...").green()
                            .appendFreshSibling(new OnyxChatComponent(EnumChatFormatting.GREEN + "  ❶ " + EnumChatFormatting.YELLOW + "add chests on your island by opening them; deselect chests by Sneaking + Right Click.").yellow())
                            .appendFreshSibling(new OnyxChatComponent(EnumChatFormatting.GREEN + "  ❷ " + EnumChatFormatting.YELLOW + "use " + EnumChatFormatting.GOLD + analyzeCommand + EnumChatFormatting.YELLOW + " again to run the chest analysis.").yellow().setSuggestCommand(analyzeCommand))
                            .appendFreshSibling(new OnyxChatComponent("     (You can search for an item inside your chests by double clicking its analysis row)").gray().setSuggestCommand(analyzeCommand))
                            .appendFreshSibling(new OnyxChatComponent(EnumChatFormatting.GREEN + "  ❸ " + EnumChatFormatting.YELLOW + "use " + EnumChatFormatting.GOLD + analyzeCommand + " stop" + EnumChatFormatting.YELLOW + " to stop the chest tracker and clear current results.").yellow().setSuggestCommand(analyzeCommand + " stop")));
                } else {
                    main.getChatHelper().sendMessage(new OnyxChatComponent("Enabled chest tracker! " + EnumChatFormatting.GRAY + "Run " + analyzeCommand + " again to run the chest analysis.").green().setSuggestCommand(analyzeCommand));
                }
            } else {
            ...
            String analyzeCommand = "/" + getCommandName() + " analyzeChests";
            main.getChatHelper().sendMessage(new OnyxChatComponent(Onyxpannel.MODNAME + " chest tracker & analyzer").gold().bold()
                    .appendFreshSibling(new OnyxChatComponent("Use " + EnumChatFormatting.GOLD + analyzeCommand + EnumChatFormatting.YELLOW + " to start tracking chests on your island! " + EnumChatFormatting.GREEN + "Then you can...").yellow().setSuggestCommand(analyzeCommand))
                    .appendFreshSibling(new OnyxChatComponent(EnumChatFormatting.GREEN + "  ❶ " + EnumChatFormatting.YELLOW + "add chests by opening them; deselect chests by Sneaking + Right Click.").yellow())
                    .appendFreshSibling(new OnyxChatComponent(EnumChatFormatting.GREEN + "  ❷ " + EnumChatFormatting.YELLOW + "use " + EnumChatFormatting.GOLD + analyzeCommand + EnumChatFormatting.YELLOW + " again to run the chest analysis.").yellow().setSuggestCommand(analyzeCommand))
                    .appendFreshSibling(new OnyxChatComponent(EnumChatFormatting.GREEN + "  ❸ " + EnumChatFormatting.YELLOW + "use " + EnumChatFormatting.GOLD + analyzeCommand + " stop" + EnumChatFormatting.YELLOW + " to stop the chest tracker and clear current results.").yellow().setSuggestCommand(analyzeCommand + " stop")));
        }
    }
                ...
                case BLOCK: {
                    ...
                    if (te instanceof TileEntitySkull) {
                        ...
                        try {
                            ...
                            main.getChatHelper().sendMessage(new OnyxChatComponent("➡ View this banner on needcoolshoes.com").green().setUrl("https://www.needcoolshoes.com/banner?=" + bannerHash));
                    ...
                    if (entity instanceof EntityArmorStand) {
                        ...
                        if (displayedItem != null) {
                            ...
                            if (displayedItem.getItem() == Items.filled_map) {
                                ...
                                if (mapFile != null) {
                                    main.getChatHelper().sendMessage(new OnyxChatComponent("Saved map as " + mapFile.getName() + " ").green().setOpenFile(mapFile).appendSibling(new OnyxChatComponent("[open file]").gold())
                                            .appendSibling(new OnyxChatComponent(" [open folder]").darkAqua().setOpenFile(mapFile.getParentFile())));
                                } else {
    ...
    private void handleDungeon(String[] args) throws OnyxCommandException {
        ...
        if (args.length == 2 && args[1].equalsIgnoreCase("enter")) {
            ...
            if (!CredentialStorage.isOnyxValid) {
                ...
                throw new OnyxCommandException("Please wait a few seconds before using this command again.");
            }
    ...
    private void handleApiKey(String[] args) throws CommandException {
        if (args.length == 1) {
            OnyxChatComponent msg;
            EnumChatFormatting color;
            EnumChatFormatting colorSecondary;
            if (CredentialStorage.isOnyxValid && StringUtils.isNotEmpty(CredentialStorage.onyx)) {
                msg = new OnyxChatComponent("[Onyxpannel] You already set your Hypixel API key.").green();
                color = EnumChatFormatting.GRAY;
                colorSecondary = EnumChatFormatting.YELLOW;
            } else {
                msg = new OnyxChatComponent("[Onyxpannel] You haven't set your Hypixel API key yet or the API key is invalid.").red();
                color = EnumChatFormatting.RED;
                colorSecondary = EnumChatFormatting.DARK_RED;
            }
            main.getChatHelper().sendMessage(msg.appendSibling(new OnyxChatComponent(color + " Use " + colorSecondary + "/" + this.getCommandName() + " apikey <key>" + color + " to manually set your API key."))
                    .appendFreshSibling(new OnyxChatComponent(" ❢ ").lightPurple().setUrl("https://github.com/onyx-mc/Onyxpannel/blob/master/CHANGELOG.md#note-on-api-keys-")
                            .appendSibling(new OnyxChatComponent("[open 'Note on API keys']").darkAqua().underline())));
        } else {
            String key = args[1];
            if (Utils.isValidUuid(key)) {
                main.getChatHelper().sendMessage(EnumChatFormatting.GREEN, "[Onyxpannel] Saved new API key.");
                main.getOnyx().setOnyx(key);
            } else {
                throw new SyntaxErrorException("[Onyxpannel] That doesn't look like a valid API key...");
            }
        }
    }
    ...
    private void handleWorldAge(String[] args) {
        if (args.length == 2) {
            ...
            OnyxConfig.notifyServerAge = enable;
    ...
    //region sub-commands: update mod
    private void handleUpdate(String[] args) throws OnyxCommandException {
        ...
        if (updateCheckStarted) {
            ...
            throw new OnyxCommandException("⚠ Update checker is on cooldown. Please wait " + EnumChatFormatting.GOLD + EnumChatFormatting.BOLD + waitingTime + EnumChatFormatting.RESET + EnumChatFormatting.RED + " more minutes before checking again.");
        }
    }
    ...
    private void handleUpdateHelp() {
        main.getChatHelper().sendMessage(new ChatComponentText("➜ Update instructions:").setChatStyle(new ChatStyle().setColor(EnumChatFormatting.GOLD).setBold(true))
                ...
                .appendSibling(new ChatComponentText("\n➌" + EnumChatFormatting.YELLOW + " copy " + EnumChatFormatting.GOLD + Onyxpannel.MODNAME.replace(" ", "") + "-" + main.getVersionChecker().getNewVersion() + ".jar" + EnumChatFormatting.YELLOW + " into mods directory").setChatStyle(new ChatStyle().setColor(EnumChatFormatting.GOLD).setBold(false)
                ...
                .appendSibling(new ChatComponentText("\n➍" + EnumChatFormatting.YELLOW + " delete old mod file " + EnumChatFormatting.GOLD + Onyxpannel.MODNAME.replace(" ", "") + "-" + Onyxpannel.VERSION + ".jar ").setChatStyle(new ChatStyle().setColor(EnumChatFormatting.GOLD).setBold(false)))
    ...
    private void sendCommandUsage(ICommandSender sender) {
        ...
        OnyxChatComponent usage = new OnyxChatComponent("➜ " + Onyxpannel.MODNAME + " commands:").gold().bold()
                ...
                .appendSibling(createCmdHelpEntry("waila", "Copy the 'thing' you're looking at (optional keybinding: Minecraft controls > Onyxpannel)"))
                ...
                .appendSibling(createCmdHelpEntry("discord", "Need help? Join the OnyxX discord"))
                ...
                .appendSibling(createCmdHelpEntry("directory", "Open Minecraft's 'mods' or Onyxpannel's 'config' directory"));
        if (main.getFriendsHandler().getBestFriendsListSize() > 0) {
            usage.appendSibling(createCmdHelpEntry("bestfriends", "§dMigrate best friends list"));
        }
        usage.appendFreshSibling(new OnyxChatComponent("➡ /commandslist " + EnumChatFormatting.YELLOW + "to list all commands added by your installed mods.").lightPurple().setSuggestCommand("/commandslist"))
                .appendFreshSibling(new OnyxChatComponent("➜ Need help with " + EnumChatFormatting.GOLD + Onyxpannel.MODNAME + EnumChatFormatting.GREEN + "? Do you have any questions, suggestions or other feedback? " + EnumChatFormatting.GOLD + "Join the OnyxX discord!").green().setUrl(Onyxpannel.INVITE_URL));
    ...
    private IChatComponent createCmdHelpEntry(String cmd, String usage) {
        ...
        return new OnyxChatComponent("\n").reset().appendSibling(new OnyxChatComponent.KeyValueChatComponent(command, usage, " ➡ ").setSuggestCommand(command));
    }
